...
<head>
    ...
    <title>{{ __('operator.auth.forgot_title') }} | {{ $operatorBranding['portal_name'] ?? 'MoxDOP' }}</title>
...
<body class="min-h-full bg-gray-50 text-gray-800 antialiased dark:bg-gray-950 dark:text-gray-100">
    <div class="flex min-h-screen flex-col items-center justify-center px-4 py-12">
        ...
        <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-sm ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('operator.auth.forgot_heading') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('operator.auth.forgot_subtitle') }}</p>
            ...
            <form method="POST" action="{{ route('app.password.email') }}" class="mt-6 space-y-4">
                @csrf
                ...
                <button type="submit"
                    ...
                    {{ __('operator.auth.send_reset') }}
                </button>
            </form>
            <p class="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
                <a href="{{ route('app.login') }}" class="font-medium text-brand-600 hover:underline">{{ __('operator.auth.back_to_login') }}</a>
            </p>
        </div>
    </div>
</body>
