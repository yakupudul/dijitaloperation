            ...
            @if ($selectedHealthPage)
                <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800" id="technical-health-detail">
                    ...
                    <div class="grid gap-px bg-gray-100 xl:grid-cols-2 dark:bg-gray-800">
                        ...
                        <div class="bg-white p-5 dark:bg-gray-900"><h4 class="font-semibold text-gray-900 dark:text-white">{{ __('operator.website.technical_health.detail.observations_title') }}</h4><div class="mt-4 space-y-2">@forelse($selectedHealthPage['issues'] as $issue)<div class="flex items-center justify-between gap-3 rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/[0.03]"><div><p class="text-sm font-medium text-gray-900 dark:text-white">{{ $issue['label'] }}</p><p class="mt-1 font-mono text-[11px] text-gray-400">{{ $issue['code'] }}</p></div><span class="shrink-0 rounded-full border px-2 py-0.5 text-xs {{ $severityTones[$issue['severity']] }}">{{ __('operator.website.technical_health.severity.'.$issue['severity']) }}</span></div>@empty<p class="text-sm text-emerald-600 dark:text-emerald-300">{{ __('operator.website.technical_health.clean') }}</p>@endforelse</div></div>
                    </div>
