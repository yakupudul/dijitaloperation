@php
    ...
    $sections = [
        ...
        'index' => $localeTr ? 'URL Inspection Sağlığı' : 'URL Inspection Health',
    ...
    $searchAppearanceStatus = $gsc['search_appearance_status'] ?? 'unavailable';
    $queryRowsAnalyzed = (int) ($gsc['data_quality']['query_rows_analyzed'] ?? 0);
    $queryRowLimit = (int) ($gsc['data_quality']['query_row_limit'] ?? 400);
    $pageRowsAnalyzed = (int) ($gsc['data_quality']['page_rows_analyzed'] ?? 0);
    $pageRowLimit = (int) ($gsc['data_quality']['page_row_limit'] ?? 150);
@endphp
...
@else
    <div x-data="{ section: 'overview', show(name) { this.section = name; this.$nextTick(() => { window.dispatchEvent(new Event('resize')); setTimeout(() => window.dispatchEvent(new Event('resize')), 80); }); } }" class="space-y-5">
        ...
        <div x-show="section === 'overview'" x-cloak class="space-y-5">
            ...
            <p class="text-xs text-gray-400">{{ $localeTr ? "Özet sayılar, provider sınırları içinde analiz edilen {$queryRowsAnalyzed} sorgu (üst sınır {$queryRowLimit}) ve {$pageRowsAnalyzed} sayfa (üst sınır {$pageRowLimit}) üzerinden hesaplanır. Aşağıdaki detay listeleri yalnızca en güçlü kayıtları gösterir." : "Summary counts are calculated over {$queryRowsAnalyzed} analyzed query rows (limit {$queryRowLimit}) and {$pageRowsAnalyzed} page rows (limit {$pageRowLimit}) within provider limits. Detail lists below show only the strongest records." }}</p>
        ...
        <div x-show="section === 'pages'" x-cloak class="space-y-5">
            ...
            <div class="grid gap-4 xl:grid-cols-3">
                @foreach ([['rising', __('website_gsc.rising'), 'text-emerald-600'],['falling', __('website_gsc.falling'), 'text-rose-600']] as [$movementKey, $movementLabel, $tone])
                    @php $movementTotal = $gsc['health_summary'][$movementKey === 'rising' ? 'rising_pages' : 'falling_pages'] ?? count($gsc['pages'][$movementKey] ?? []); @endphp
                    <section class="rounded-2xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800"><div class="flex items-center justify-between"><h3 class="text-sm font-semibold {{ $tone }}">{{ $movementLabel }}</h3><span class="text-xs text-gray-400">{{ $movementTotal }}</span></div><div class="mt-3 space-y-2">@forelse (array_slice($gsc['pages'][$movementKey] ?? [], 0, 8) as $row)<div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/[0.03]"><p class="truncate text-xs font-medium text-gray-800 dark:text-gray-200" title="{{ $row['page'] ?? '' }}">{{ $row['page'] ?? '—' }}</p><p class="mt-1 text-[11px] text-gray-500">{{ $formatNumber($row['clicks'] ?? 0) }} · {{ $formatNumber($row['impressions'] ?? 0) }}</p></div>@empty<p class="text-sm text-gray-500">—</p>@endforelse</div></section>
                @endforeach
        ...
        <div x-show="section === 'audience'" x-cloak class="space-y-5">
            ...
            @if (collect($gsc['cross_dimensions'] ?? [])->flatten(1)->isNotEmpty())
            ...
            <section class="overflow-hidden rounded-2xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                ...
                @if ($searchAppearanceStatus === 'unavailable')
                    <div class="px-5 py-8"><p class="text-sm font-medium text-amber-700 dark:text-amber-300">{{ $localeTr ? 'Search Appearance veri kaynağı henüz hazır değil.' : 'The Search Appearance dataset is not available yet.' }}</p><p class="mt-1 text-xs text-gray-500">{{ $localeTr ? 'Bu durum “Google’da sonuç yok” anlamına gelmez; bu kaynak için güvenilir veri seti henüz toplanmamış olabilir.' : 'This does not mean Google has no results; a reliable provider dataset may not have been collected for this source yet.' }}</p></div>
                @elseif ($searchAppearanceStatus === 'empty')
                    <div class="px-5 py-8 text-sm text-gray-500">{{ $localeTr ? 'Search Appearance veri seti hazır, ancak seçili dönemde kayıt yok.' : 'The Search Appearance dataset is available, but there are no rows for the selected period.' }}</div>
                @else
                    <div class="grid gap-0 md:grid-cols-2 xl:grid-cols-3">@foreach (array_slice($gsc['search_appearances'] ?? [], 0, 12) as $row)<div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800 md:border-r"><p class="truncate text-sm font-medium text-gray-800 dark:text-gray-200">{{ $row['appearance'] }}</p><p class="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{{ $formatNumber($row['clicks']) }}</p><p class="mt-1 text-xs text-gray-500">{{ $formatNumber($row['impressions']) }} · {{ $formatPercent($row['ctr']) }}</p></div>@endforeach</div>
                @endif
            </section>
        </div>

        {{-- 6. URL Inspection Sağlığı --}}
        <div x-show="section === 'index'" x-cloak class="space-y-5">
            <div class="grid gap-4 xl:grid-cols-2">
                ...
                <section class="rounded-2xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ $localeTr ? 'URL Inspection örnek sağlığı' : 'URL Inspection sample health' }}</h3>
                    <p class="mt-1 text-xs text-gray-500">{{ $localeTr ? 'Bu kart tüm sitenin index coverage durumunu değil, merkezi havuzdaki kontrollü URL Inspection örneğini özetler.' : 'This card summarizes the controlled URL Inspection sample in the central pool, not site-wide index coverage.' }}</p>
                    @if ($gsc['index_health']['available'] ?? false)<div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4"><div class="rounded-xl bg-gray-50 p-3 text-center dark:bg-white/[0.03]"><p class="text-xl font-semibold text-gray-900 dark:text-white">{{ $gsc['index_health']['total'] }}</p><p class="text-[11px] text-gray-500">{{ $localeTr ? 'İncelenen örnek' : 'Inspected sample' }}</p></div><div class="rounded-xl bg-gray-50 p-3 text-center dark:bg-white/[0.03]"><p class="text-xl font-semibold text-emerald-600">{{ $gsc['index_health']['indexable'] }}</p><p class="text-[11px] text-gray-500">PASS</p></div><div class="rounded-xl bg-gray-50 p-3 text-center dark:bg-white/[0.03]"><p class="text-xl font-semibold text-rose-600">{{ $gsc['index_health']['issues'] }}</p><p class="text-[11px] text-gray-500">{{ $localeTr ? 'Örnek sorunu' : 'Sample issues' }}</p></div><div class="rounded-xl bg-gray-50 p-3 text-center dark:bg-white/[0.03]"><p class="text-xl font-semibold text-amber-600">{{ $gsc['index_health']['canonical_mismatches'] }}</p><p class="text-[11px] text-gray-500">Canonical</p></div></div><div class="mt-4 space-y-2">@foreach (array_slice($gsc['url_inspection'] ?? [], 0, 10) as $row)<div class="rounded-lg bg-gray-50 p-3 text-xs dark:bg-white/[0.03]"><p class="truncate font-medium text-gray-800 dark:text-gray-200">{{ $row['page'] }}</p><p class="mt-1 text-gray-500">{{ $row['verdict'] ?? '—' }} · {{ $row['coverage_state'] ?? '—' }} · {{ $row['last_crawl_time'] ?? '—' }}</p>@if (! empty($row['google_canonical']) && ! empty($row['user_canonical']) && $row['google_canonical'] !== $row['user_canonical'])<p class="mt-1 text-amber-700 dark:text-amber-300">{{ __('website_gsc.canonical_mismatch') }}: Google → {{ $row['google_canonical'] }}</p>@endif</div>@endforeach</div>@else<div class="mt-4 rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]"><p class="text-sm text-gray-600 dark:text-gray-300">{{ __('website_gsc.inspection_unavailable') }}</p></div>@endif
                </section>
            </div>
        ...
        <div x-show="section === 'actions'" x-cloak class="space-y-5">
            <div class="rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-600 ring-1 ring-inset ring-gray-100 dark:bg-white/[0.03] dark:text-gray-300 dark:ring-gray-800">
                {{ $localeTr ? count($gsc['risks'] ?? []).' otomatik sinyal · '.$openFindings->count().' açık bulgu · '.$recommendations->count().' aktif öneri. Sinyal, doğrulanmış bulgu değildir; operasyon kaydına dönüştürülmesi gerekir.' : count($gsc['risks'] ?? []).' automated signals · '.$openFindings->count().' open findings · '.$recommendations->count().' active recommendations. A signal is not a validated finding until it is promoted into the operations workflow.' }}
            </div>
            <div class="grid gap-4 xl:grid-cols-2">
