    ...
    <div x-data="{ section: 'overview', show(name) { this.section = name; this.$nextTick(() => { window.dispatchEvent(new Event('resize')); setTimeout(() => window.dispatchEvent(new Event('resize')), 80); }); } }" class="space-y-5">
        ...
        <div x-show="section === 'audience'" x-cloak class="space-y-5">
            ...
            <section class="overflow-hidden rounded-2xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                ...
                @if ($searchAppearanceStatus === 'unavailable')
                    <div class="px-5 py-8"><p class="text-sm font-medium text-amber-700 dark:text-amber-300">{{ $localeTr ? 'Search Appearance veri kaynağı henüz hazır değil.' : 'The Search Appearance dataset is not available yet.' }}</p><p class="mt-1 text-xs text-gray-500">{{ $localeTr ? 'Bu durum “Google’da sonuç yok” anlamına gelmez; bu kaynak için güvenilir provider dataset henüz toplanmamış olabilir.' : 'This does not mean Google has no results; a reliable provider dataset may not have been collected for this source yet.' }}</p></div>
                @elseif ($searchAppearanceStatus === 'empty')
                    <div class="px-5 py-8 text-sm text-gray-500">{{ $localeTr ? 'Search Appearance dataseti hazır, ancak seçili dönemde kayıt yok.' : 'The Search Appearance dataset is available, but there are no rows for the selected period.' }}</div>
                @else
        ...
        <div x-show="section === 'ai'" x-cloak class="space-y-5">
            ...
            <section class="rounded-2xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800"><div class="flex items-start justify-between gap-4"><div><h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ $localeTr ? 'Zengin sonuç sinyalleri' : 'Rich-result signals' }}</h3><p class="mt-1 text-sm text-gray-500">{{ $localeTr ? 'Search Appearance, AI verisi değildir; ancak özel Google sonuç türlerinin gerçek performansını gösterir.' : 'Search Appearance is not AI data; it shows real performance of special Google result types.' }}</p></div><span class="rounded-full bg-gray-50 px-2.5 py-1 text-xs font-medium text-gray-500 dark:bg-white/[0.04]">{{ count($gsc['search_appearances'] ?? []) }}</span></div>
                @if ($searchAppearanceStatus === 'unavailable')
                    <p class="mt-4 text-sm text-amber-700 dark:text-amber-300">{{ $localeTr ? 'Search Appearance provider dataseti henüz hazır değil.' : 'The Search Appearance provider dataset is not available yet.' }}</p>
                @elseif ($searchAppearanceStatus === 'empty')
                    <p class="mt-4 text-sm text-gray-500">{{ $localeTr ? 'Dataset hazır, seçili dönemde Search Appearance kaydı yok.' : 'The dataset is available, but there are no Search Appearance rows for this period.' }}</p>
                @else
