...
<div class="space-y-5">
    ...
    <section class="{{ $card }}">
        ...
        @else
            <div class="mt-2 overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="text-left text-xs uppercase text-gray-400"><tr><th class="py-2 pr-3">Hesap</th><th class="py-2 pr-3">Durum</th><th class="py-2 pr-3">Veri tarihi</th><th class="py-2 pr-3">Son başarılı</th><th class="py-2">Sonraki</th></tr></thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach ($health['accounts'] as $account)
                            <tr wire:key="acc-{{ $account['id'] }}">
                                <td class="py-2 pr-3 text-gray-800 dark:text-gray-200">{{ $account['name'] }} <span class="text-xs text-gray-400">{{ $account['type'] }}</span></td>
                                <td class="py-2 pr-3 text-xs">
                                    @if (! $account['enabled'])
                                        <span class="text-gray-400">Kapalı</span>
                                    @elseif ($account['state'] === 'attention')
                                        <span class="text-rose-600">{{ $errorLabels[$account['error']] ?? ($account['error'] ?: 'Dikkat') }}</span>
                                    @else
                                        {{ $account['state'] }}
                                    @endif
                                ...
                                <td class="py-2 text-xs">{{ $when($account['next']) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
