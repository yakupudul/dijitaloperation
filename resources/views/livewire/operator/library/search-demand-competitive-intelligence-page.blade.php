        ...
        <div class="space-y-5">
            @foreach($run->analyses as $analysis)
                <article class="rounded-xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                    ...
                    @if($analysis->standard_assessments)
                        <div class="mt-4 overflow-x-auto"><h3 class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">Ortak standartlarla karşılaştırma</h3>
                            <p class="mb-2 text-xs text-gray-500">{{ ['comparable' => 'Benzer amaç ve sayfa türü', 'different_intent' => 'Arama niyetleri farklı', 'different_page_type' => 'Sayfa türleri farklı', 'unknown' => 'Karşılaştırılabilirlik belirsiz'][$analysis->comparability] ?? 'Karşılaştırılabilirlik belirsiz' }}</p>
                            <table class="w-full text-left text-sm"><thead class="text-gray-500"><tr><th class="p-2">Kriter</th><th class="p-2">Bizim sayfamız</th><th class="p-2">Rakip sayfa</th><th class="p-2">Gerekçe</th></tr></thead><tbody>
                            @foreach($analysis->standard_assessments as $criterion)
                                <tr class="border-t border-gray-100 text-gray-600 dark:border-gray-800 dark:text-gray-400"><td class="p-2">{{ collect(data_get($run->input_payload, 'standards', []))->firstWhere('id', $criterion['standard_id'])['title'] ?? $criterion['standard_id'] }}</td><td class="p-2">{{ ['pass'=>'Uygun','gap'=>'Eksik','partial'=>'Kısmi','unknown'=>'Belirsiz','not_applicable'=>'Uygulanmaz'][$criterion['brand_state']] }}<p class="mt-1 text-xs">{{ $criterion['brand_evidence'] }}</p></td><td class="p-2">{{ ['pass'=>'Uygun','gap'=>'Eksik','partial'=>'Kısmi','unknown'=>'Belirsiz','not_applicable'=>'Uygulanmaz'][$criterion['competitor_state']] }}<p class="mt-1 text-xs">{{ $criterion['competitor_evidence'] }}</p></td><td class="p-2">{{ $criterion['rationale'] }}</td></tr>
                            @endforeach
                            </tbody></table>
                        </div>
                    @endif
