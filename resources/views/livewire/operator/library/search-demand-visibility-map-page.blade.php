...
<div class="space-y-6">
    ...
    @if ($website)
                        ...
                        @forelse($result['rows'] as $row)
                            <tr class="align-top hover:bg-gray-50/60 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="max-w-md px-4 py-3">
                                    ...
                                    <div class="mt-1 break-all text-[10px] text-brand-600">Hedef: {{ data_get($row, 'ownership.target_url', 'atanmadı') ?: 'atanmadı' }} · {{ data_get($row, 'ownership.status', 'unassigned') }}</div>
        ...
        <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            ...
            <div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="bg-gray-50 text-left text-gray-500 dark:bg-white/[0.02]"><tr><th class="px-4 py-3">Küme</th><th class="px-4 py-3">Hedef URL</th><th class="px-3 py-3 text-right">Sorgu</th><th class="px-3 py-3 text-right">Gözlemlenen</th><th class="px-3 py-3 text-right">URL</th><th class="px-3 py-3 text-right">Tıklama</th><th class="px-4 py-3 text-right">Gösterim</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-gray-800">@forelse($result['cluster_summary'] as $cluster)<tr><td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">{{ $cluster['cluster_name'] }}</td><td class="max-w-sm px-4 py-3"><p class="break-all">{{ $cluster['target_url'] ?: '—' }}</p><p class="text-[10px] text-gray-400">{{ $cluster['ownership_status'] }}</p></td><td class="px-3 py-3 text-right">{{ $number($cluster['query_count']) }}</td><td class="px-3 py-3 text-right">{{ $number($cluster['observed_query_count']) }}</td><td class="px-3 py-3 text-right">{{ $number($cluster['url_count']) }}</td><td class="px-3 py-3 text-right">{{ $number($cluster['clicks']) }}</td><td class="px-4 py-3 text-right">{{ $number($cluster['impressions']) }}</td></tr>@empty<tr><td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">Küme özeti yok.</td></tr>@endforelse</tbody></table></div>
        </section>
    @endif
</div>
