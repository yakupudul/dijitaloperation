                ...
                <table class="w-full text-left text-sm">
                    ...
                    @forelse($accounts as $a)
                        ...
                        <tr wire:key="auto-account-{{ $a->id }}" class="align-top">
                            ...
                            @endif
                            <td class="space-y-2 p-3 text-right text-xs">@if($isAdmin)<button type="button" wire:click="edit({{ $a->id }})" class="block w-full text-brand-600">{{ __('resource-auto.settings') }}</button><button type="button" wire:click="runNow({{ $a->id }})" wire:loading.attr="disabled" @disabled($problem || $active || in_array($a->collection_status, ['planning','collecting'])) class="block w-full disabled:opacity-40">{{ __('resource-auto.run_now') }}</button>@endif@if($showQueryColumns && in_array($a->resource->resource_type, ['google_ads','search_console'], true))<button type="button" wire:click="details({{ $a->id }})" class="block w-full">{{ __('resource-auto.details') }}</button>@endif<button type="button" wire:click="inspectCollection({{ $a->id }})" class="block w-full text-brand-600">{{ __('resource-auto.inspect_collection') }}</button></td>
                        </tr>
        ...
        <div class="fixed inset-0 z-[100] flex justify-end bg-black/40" role="dialog" aria-modal="true" aria-label="{{ __('resource-auto.settings') }}" x-data x-trap.inert.noscroll="true" x-on:keydown.escape.window="$wire.closeEditor()">
            <form wire:submit="save" class="flex h-full w-full max-w-lg flex-col bg-white shadow-xl dark:bg-gray-900">
                ...
                <div class="flex-1 space-y-5 overflow-y-auto p-5">
                    ...
                    <label class="block text-sm">Çekim saati (İstanbul)<select wire:model="preferredHour" class="mt-2 w-full rounded-lg border-gray-300 text-sm dark:border-gray-700 dark:bg-gray-950"><option value="">Otomatik (son çekimden itibaren)</option>@for ($h = 0; $h < 24; $h++)<option value="{{ $h }}">{{ str_pad((string) $h, 2, '0', STR_PAD_LEFT) }}:00</option>@endfor</select></label>
    ...
    @if($detail)
        <div class="fixed inset-0 z-[100] overflow-y-auto bg-black/40 p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="{{ __('resource-auto.details') }}" x-data x-trap.inert.noscroll="true" x-on:keydown.escape.window="$wire.closeDetails()">
            <div class="mx-auto max-w-5xl space-y-4 rounded-xl bg-white p-5 dark:bg-gray-900">
                ...
                @if($detail->sector && $isAdmin)<button type="button" wire:click="recheck({{ $detail->id }})" wire:confirm="{{ __('resource-auto.recheck_confirm') }}" wire:loading.attr="disabled" class="rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700">{{ __('resource-auto.recheck') }}</button>@endif
