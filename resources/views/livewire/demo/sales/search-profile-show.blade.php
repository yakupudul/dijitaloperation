<div class="space-y-5">
    ...
    <x-ta.card>
        ...
        <p class="mt-3 text-sm text-gray-500">{{ __('free_radar.free_profile_note') }}</p>
        <button type="button" wire:click="runSearch" wire:loading.attr="disabled"
            class="mt-4 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-40">{{ __('free_radar.check') }}</button>
        <a href="{{ route('operator.intent-radar') }}" class="ml-3 text-sm text-brand-600">{{ __('free_radar.title') }}</a>
    </x-ta.card>
