<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">Customers</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Portfolio customers in Demo Mode (session-backed).</p>
        </div>
        <x-ta.button wire:click="openAdd" size="sm">Add customer</x-ta.button>
    </div>
    ...
    <x-ta.table>
        ...
        @foreach ($customers as $customer)
            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                <td class="px-5 py-4 text-sm font-medium text-gray-800 dark:text-white/90">{{ $customer['name'] }}</td>
                ...
                <td class="px-5 py-4"><x-ta.badge color="success" size="sm">{{ $customer['status'] }}</x-ta.badge></td>
    ...
    <div x-data="{ open: @entangle('showAdd') }">
        <x-ta.modal>
            ...
            <p class="mt-1 text-sm text-gray-500">Demo Mode — stored in session only.</p>
            <div class="mt-4 space-y-3">
                ...
                <div>
                    ...
                    <input wire:model="industry" type="text" class="w-full rounded-lg border border-gray-200 bg-transparent px-3 py-2 text-sm dark:border-gray-700" />
                </div>
                <div>
                    <label class="mb-1 block text-sm text-gray-600 dark:text-gray-300">HQ</label>
                    <input wire:model="hq" type="text" class="w-full rounded-lg border border-gray-200 bg-transparent px-3 py-2 text-sm dark:border-gray-700" />
                </div>
            </div>
            <div class="mt-6 flex justify-end gap-2">
                <x-ta.button wire:click="closeAdd" variant="outline" size="sm">Cancel</x-ta.button>
                <x-ta.button wire:click="saveCustomer" size="sm">Save</x-ta.button>
            </div>
        </x-ta.modal>
    </div>
</div>
