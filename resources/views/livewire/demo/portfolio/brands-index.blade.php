<div class="space-y-6">
    ...
    @include('livewire.demo.partials.workspace-header', [
        ...
        'actions' => '<a href="'.route('demo.brand.create').'" wire:navigate class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">Add brand</a>',
    ])
            ...
            <x-ta.card>
                <div class="flex items-start justify-between gap-2">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ $brand['location'] ?? '—' }} · {{ $brand['sector_label'] ?? '' }}</p>
                    </div>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    ...
                    <x-ta.button :href="route('demo.brand.edit', ['brandId' => $brand['id']])" size="sm" variant="outline">Edit</x-ta.button>
                    <x-ta.button :href="route('demo.asset.create', ['brandId' => $brand['id']])" size="sm" variant="outline">Add asset</x-ta.button>
                </div>
            </x-ta.card>
        @empty
