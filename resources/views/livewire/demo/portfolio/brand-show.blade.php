        ...
        <div class="space-y-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Review public website context before accepting it into the brand's factual business context.</p>
                    <p class="mt-2 text-xs text-gray-400">V1 is bounded Website public discovery — not a universal internet crawler.</p>
                </div>
                <button type="button" wire:click="runPublicResearch" @disabled(count($websites) === 0 || (count($websites) > 1 && $website_id === '')) class="inline-flex rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                    Discover public context
                </button>
            </div>

            @if (count($websites) === 0)
                <div class="rounded-xl bg-white p-6 text-sm text-gray-500 ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                    No Website digital asset under this brand. Add a Website asset before running Public Discovery.
                </div>
            @else
                @if (count($websites) > 1)
                    <div>
                        <label for="website-inspect" class="mb-1 block text-xs font-medium text-gray-500">Website to inspect</label>
                        <select id="website-inspect" wire:model.live="website_id" class="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                ...
                @if (! empty($research['completed']))
                    <div class="rounded-xl bg-white p-4 text-sm ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                        <p class="font-medium text-gray-800 dark:text-white/90">Last discovery</p>
                        <p class="mt-1 text-gray-500">Completed {{ $research['completed_at'] ?? 'recently' }}</p>
                        <p class="mt-1 text-gray-500">Website · {{ $research['website'] ?? ($selectedWebsite['name'] ?? '—') }}</p>
                        @if (! empty($research['pages_inspected']))
                            <p class="mt-1 text-gray-500">Pages inspected · {{ $research['pages_inspected'] }}</p>
                        @endif
                    </div>
                @endif
                ...
                @php
                    $groups = [
                        ...
                        'inference' => 'AI-derived interpretations',
                        'competitor' => 'Competitor candidates',
                    ];
                @endphp
                @foreach ($groups as $kind => $groupLabel)
                    @php $groupRows = collect($discoveryCandidates)->where('kind', $kind)->values(); @endphp
                    @if ($groupRows->isNotEmpty())
                        <section>
                            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ $groupLabel }}</h3>
                            <div class="mt-2 space-y-2">
                                @foreach ($groupRows as $candidate)
                                    <div class="rounded-xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:ring-gray-700" wire:key="dc-{{ $candidate['id'] }}">
                                        <div class="flex flex-wrap items-start justify-between gap-3">
                                            <div>
                                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $candidate['value'] }}</p>
                                                <p class="mt-1 text-xs text-gray-500">{{ $candidate['type'] }} · {{ $candidate['kind_label'] }} · {{ $candidate['status'] }}</p>
                                                <p class="mt-1 text-xs text-gray-400">Source: {{ $candidate['source'] }} · Retrieved {{ $candidate['retrieved'] }}</p>
                                                @if (! empty($candidate['confidence']))
                                                    <p class="mt-1 text-xs text-gray-400">Confidence: {{ $candidate['confidence'] }}</p>
                                                @endif
                                            </div>
                                            @if (($candidate['status'] ?? '') === 'pending')
                                                <div class="flex flex-wrap gap-2">
                                                    <button type="button" wire:click="acceptDiscoveryCandidate('{{ $candidate['id'] }}')" class="rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-medium text-white">Accept</button>
                                                    <button type="button" wire:click="editAcceptDiscoveryCandidate('{{ $candidate['id'] }}')" class="rounded-lg px-3 py-1.5 text-xs font-medium ring-1 ring-inset ring-gray-300 dark:ring-gray-700">Edit &amp; Accept</button>
                                                    <button type="button" wire:click="ignoreDiscoveryCandidate('{{ $candidate['id'] }}')" class="rounded-lg px-3 py-1.5 text-xs font-medium text-gray-500 hover:underline">Ignore</button>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                ...
                @if (empty($research['completed']) && collect($discoveryCandidates)->where('status', 'pending')->isEmpty())
                    <p class="text-sm text-gray-500">Run Discover public context to populate candidates for human review.</p>
                @endif
            @endif
        </div>
    @endif
