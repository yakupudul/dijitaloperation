...
<div class="space-y-5">
        ...
        <header class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex min-w-0 items-start gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">GA4 mülklerinizi seçin; MoxDOP ilk aktarımı, güncellemeyi ve eksik veri onarımını otomatik belirlesin.</p>
                </div>
            </div>
            ...
            <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                <div class="flex flex-col gap-3 border-b border-gray-100 px-4 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-0.5 max-w-3xl text-sm text-gray-500 dark:text-gray-400">İlk aktarım 486 günlük geçmişi alır. Sonraki işlemlerde eksik dönem tamamlanır ve son 14 gün yeniden doğrulanır.</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        >
                            <span wire:loading.remove wire:target="collectSelectedGa4">{{ $primaryAction }}</span>
                            ...
                            <div class="flex shrink-0 items-center gap-3 pl-7 sm:pl-0">
                                <div class="min-w-[190px] text-left sm:text-right">
                                    @switch($resource['data_state'] ?? 'not_collected')
                                        @case('collecting')
                                            <p class="text-sm font-medium text-blue-700 dark:text-blue-300">Çekiliyor</p>
                                            <p class="mt-0.5 text-xs text-gray-400">Canlı ilerleme yukarıda</p>
                                            @break
                                        @case('needs_repair')
                                            <p class="text-sm font-medium text-amber-700 dark:text-amber-300">Eksik veri var</p>
                                            <p class="mt-0.5 text-xs text-gray-400">{{ $resource['failed_datasets'] }} veri grubu · {{ $resource['collection_action'] }}</p>
                                            @break
                                        @case('resume')
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Aktarım durduruldu</p>
                                            <p class="mt-0.5 text-xs text-gray-400">{{ $resource['collection_action'] }}</p>
                                            @break
                                        @case('collected')
                                            <p class="text-sm font-medium text-emerald-700 dark:text-emerald-400">Veri mevcut</p>
                                            @if (! empty($resource['coverage_start']) && ! empty($resource['coverage_end']))
                                                <p class="mt-0.5 text-xs text-gray-400">{{ $resource['coverage_start'] }} → {{ $resource['coverage_end'] }}</p>
                                            @endif
                                            <p class="mt-0.5 text-xs text-gray-400">Son başarılı aktarım {{ $resource['last_success'] }}</p>
                                            @break
                                        @default
                                            <p class="text-sm font-medium text-gray-500">Henüz veri yok</p>
                                            <p class="mt-0.5 text-xs text-gray-400">İlk aktarım · 486 gün</p>
                                    @endswitch
                                </div>
                ...
                <div class="grid flex-1 gap-2 sm:grid-cols-3">
                    ...
                    <div>
                        ...
                        <select id="res-state" wire:model.live="state" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-950 dark:text-white/90">
                            <option value="all">All</option><option value="bound">Bound</option><option value="available">Available</option><option value="conflict">Conflict</option><option value="unavailable">Unavailable</option>
                        </select>
                    </div>
                    <div>
                        <label for="res-brand" class="mb-1 block text-xs font-medium text-gray-500">Brand mapping</label>
                        <select id="res-brand" wire:model.live="brand" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-950 dark:text-white/90"><option value="all">All</option><option value="unmapped">Unmapped</option></select>
                    </div>
                </div>
                ...
                @forelse ($resources as $resource)
                    <li class="rounded-xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800" wire:key="res-{{ $resource['id'] }}">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2"><h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ $resource['name'] }}</h3><span class="text-[11px] font-semibold uppercase text-gray-400">{{ $resource['state_label'] }}</span></div>
        ...
        <div class="space-y-3">
            @forelse ($bindings as $binding)
                <article class="rounded-xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                    ...
                    <div class="mt-1 flex flex-wrap items-center justify-between gap-2"><p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $binding['asset_name'] }}</p>@if (! empty($binding['asset_url']))<a href="{{ $binding['asset_url'] }}" wire:navigate class="text-xs font-medium text-brand-600 hover:underline dark:text-brand-400">Open Digital Asset →</a>@endif</div>
                </article>
            @empty
    ...
    @if ($displayTab === 'data')
        @if ($isGa4)
            <section class="rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800"><h2 class="text-base font-semibold text-gray-900 dark:text-white">Merkezi veri havuzu</h2><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">GA4'ten alınan verilerin merkezi durumunu görüntüleyin.</p></div>
                <dl class="grid divide-y divide-gray-100 dark:divide-gray-800 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                    @foreach ($data['data']['metrics'] as $metric)
                        <div class="px-5 py-4"><dt class="text-xs text-gray-400">{{ $metric['label'] }}</dt><dd class="mt-1 text-xl font-semibold text-gray-900 dark:text-white">{{ $metric['value'] }}</dd><dd class="mt-0.5 text-xs text-gray-500">{{ $metric['state'] }}</dd></div>
                    @endforeach
                </dl>
        ...
        @else
            <section class="rounded-xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                ...
                <dl class="mt-4 grid gap-3 sm:grid-cols-2">@foreach ($data['data']['metrics'] as $metric)<div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/[0.03]"><dt class="text-xs text-gray-400">{{ $metric['label'] }}</dt><dd class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $metric['value'] }}</dd><dd class="text-[11px] text-gray-500">{{ $metric['state'] }}</dd></div>@endforeach</dl>
    ...
    @if ($displayTab === 'activity')
        @if ($isGa4)
            <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
                <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800"><h2 class="text-base font-semibold text-gray-900 dark:text-white">Aktarım geçmişi</h2><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Son GA4 toplama hareketleri.</p></div>
                <div class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($data['activity'] as $event)
                        ...
                        <div class="flex flex-col gap-2 px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2"><span @class(['h-2 w-2 rounded-full','bg-emerald-500' => $isSuccess,'bg-amber-500' => $isProblem,'bg-gray-400' => $isStopped,'bg-blue-500' => ! $isSuccess && ! $isProblem && ! $isStopped])></span><p class="text-sm font-medium text-gray-900 dark:text-white">{{ $event['event'] }}</p></div>
        ...
        @else
            <ol class="space-y-3">
                @forelse ($data['activity'] as $event)
                    <li class="rounded-xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800"><p class="text-[11px] text-gray-400">{{ $event['when'] }} · {{ $event['actor'] }} · {{ $event['kind'] }}</p><p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $event['event'] }}</p><p class="text-xs text-gray-500">{{ $event['detail'] }}</p></li>
                @empty
