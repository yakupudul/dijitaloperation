        ...
        <article class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="mt-4 space-y-3">
                @forelse ($topCampaigns as $campaign)
                    ...
                    <div class="flex items-center justify-between gap-4 border-b border-gray-100 pb-3 last:border-0 last:pb-0 dark:border-gray-800"><div class="min-w-0"><p class="truncate text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $campaign['name'] }}</p><p class="mt-0.5 text-xs text-gray-400">{{ $isTr ? 'Tıklama oranı' : 'Click rate' }} {{ $campaign['ctr'] !== null ? number_format($campaign['ctr'], 2).'%' : '—' }} · {{ $statusLabel($campaign['effective_status'] ?? $campaign['status'] ?? null) }}</p>@if($action)<p class="mt-1 text-[11px] font-medium text-brand-600 dark:text-brand-400">{{ $isTr ? $action['label_tr'] : $action['label_en'] }}: {{ number_format(round((float) $action['value'])) }}</p>@endif</div><span class="shrink-0 text-sm font-bold text-gray-900 dark:text-white">{{ $campaign['spend_display'] }}</span></div>
                @empty
