...
<section class="space-y-5">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ($cards as $card)
            @php
                ...
                $delta = ($compare ?? true) ? ($metric['delta_pct'] ?? null) : null;
    ...
    <div class="grid gap-5 xl:grid-cols-2">
        <article class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
            <div class="flex items-start justify-between gap-3"><div><h3 class="font-bold text-gray-900 dark:text-white">{{ $isTr ? 'Reklamların Ürettiği Ana Sonuçlar' : 'Headline Ad Outcomes' }}</h3><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $isTr ? 'Aynı olayı tekrar eden teknik varyantlar burada çoğaltılmaz; tüm sonuçların listesi Dönüşümler sekmesindedir.' : 'Overlapping technical variants are de-duplicated here; the detailed action list remains in Conversions.' }}</p></div><button type="button" wire:click="setTab('measurement')" class="text-xs font-semibold text-brand-600 dark:text-brand-400">{{ $isTr ? 'Detay' : 'Details' }}</button></div>
            <div class="mt-5 grid gap-3 sm:grid-cols-2">
                @forelse ($actions as $action)
                    <div class="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><p class="text-xs text-gray-500 dark:text-gray-400">{{ $isTr ? ($action['label_tr'] ?? $action['label']) : ($action['label_en'] ?? $action['label']) }}</p><p class="mt-1 text-2xl font-bold tabular-nums text-gray-900 dark:text-white">{{ number_format(round((float) $action['value'])) }}</p></div>
                @empty
