    ...
    <section class="rounded-xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800" aria-labelledby="weekly-top-heading">
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex gap-3">
                <a href="{{ route('operator.seo_tasks') }}" wire:navigate class="text-xs font-medium text-brand-600 hover:underline dark:text-brand-400">{{ __('operator.nav.seo_tasks') }}</a>
                <a href="{{ route('operator.ads_advisor') }}" wire:navigate class="text-xs font-medium text-brand-600 hover:underline dark:text-brand-400">{{ __('operator.nav.ads_advisor') }}</a>
            </div>
        </div>
        @if ($weeklyTop === [])
                    ...
                    <li class="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/[0.03]">
                        <div class="flex min-w-0 flex-1 items-start gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-gray-500">{{ $row['brand'] ?? '—' }} · {{ $row['channel'] }}@if ($row['impact']) · {{ $row['impact'] }}@endif</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
