<div class="flex flex-wrap gap-2">
    ...
    <button type="button" wire:click="runLocalVisibilityScan"
        ...
        title="Local visibility scan requires a configured rank provider">
        Run local visibility scan
    </button>
    <details class="relative">
        ...
        <div class="absolute right-0 z-20 mt-2 w-56 rounded-xl bg-white p-2 shadow-lg ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-gray-700">
            ...
            @endif
            <a href="{{ route('operator.asset.create', ['brandId' => $identity['brand_id']]) }}" wire:navigate class="block rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-white/[0.04]">Edit Digital Asset</a>
            <button type="button" wire:click="setTab('profile')" class="block w-full rounded-lg px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-white/[0.04]">Manage connection</button>
        </div>
    </details>
</div>
