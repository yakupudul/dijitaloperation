@php
    ...
    $decisionReasons = $isTr ? [
        ...
        'insufficient' => 'Sağlıklı bir bütçe kararı için dönüşüm ve maliyet verisi yetersiz.',
...
<div class="space-y-5">
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        @foreach ([
            ...
            [$isTr ? 'Google Ads dönüşümü' : 'Provider conversions', $number($summary['provider_conversions'] ?? null, 2), null],
            [$isTr ? 'Dönüşüm başı maliyet' : 'Provider CPA', $money($summary['provider_cpa'] ?? null), $isTr ? 'Nitelikli müşteri adayı maliyeti değildir' : 'Not qualified-lead CPA'],
            [$isTr ? 'Karar CPA ölçütü' : 'Decision CPA benchmark', $money($summary['benchmark_cpa'] ?? null), ($summary['benchmark_source'] ?? '') === 'plan_target_cpa' ? ($isTr ? 'Ajans hedef CPA' : 'Agency target CPA') : ($isTr ? 'Hesabın ortalama CPA değeri' : 'Account provider CPA average')],
    ...
    <section class="rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
        <div class="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">{{ $isTr ? 'Google performansı, bütçe/sıralama kaybı ve varsa ajans hedef maliyeti birlikte değerlendirilir.' : 'Combines provider performance, budget/rank loss and the agency target CPA when available.' }}</p>
            </div>
            ...
            <table class="min-w-[1250px] w-full text-sm">
                <thead class="bg-gray-50 text-xs text-gray-500 dark:bg-white/[0.02]">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right">{{ $isTr ? 'Dönüşüm başı maliyet' : 'Provider CPA' }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($campaigns as $row)
                        ...
                        <tr class="align-top">
                            <td class="px-4 py-3"><p class="font-medium text-gray-800 dark:text-gray-200">{{ $row['name'] }}</p><p class="mt-0.5 text-[11px] text-gray-400">{{ $row['type'] }} · {{ \App\Services\GoogleAds\Support\GoogleAdsDisplayFormat::status((string) ($row['status'] ?? '')) }}</p></td>
    ...
    <section class="grid gap-4 xl:grid-cols-2">
        <div class="rounded-xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <div>
                <h3 class="font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Bütçe fırsat haritası' : 'Budget opportunity map' }}</h3>
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2">
                @foreach ([
                    ['scale', $isTr ? 'Büyüt' : 'Scale', 'bg-emerald-50/70 ring-emerald-100 dark:bg-emerald-500/[0.06] dark:ring-emerald-500/20'],
                    ...
                    ['reduce', $isTr ? 'Azalt / sıralamayı incele' : 'Reduce / inspect rank', 'bg-rose-50/70 ring-rose-100 dark:bg-rose-500/[0.06] dark:ring-rose-500/20'],
        ...
        <div class="rounded-xl bg-white p-5 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            ...
            <p class="mt-3 text-[11px] leading-4 text-gray-400">{{ $isTr ? 'Kesin tutar önerisi için Google’ın bütçe tahmin verisi gerekir. MOXDOP bu veri olmadan ek dönüşüm veya maliyet tahmini üretmez.' : 'Exact transfer amounts require forecast/simulator evidence. MOXDOP does not invent marginal CPA or incremental conversions without it.' }}</p>
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
        <div class="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">{{ $isTr ? 'Google’daki teklif stratejilerini kullanım durumu ve hedefleriyle birlikte gösterir.' : 'Translates raw provider strategies into operator-friendly health and target context.' }}</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-[900px] w-full text-sm">
                <thead class="bg-gray-50 text-xs text-gray-500 dark:bg-white/[0.02]"><tr><th class="px-4 py-2 text-left">{{ $isTr ? 'Strateji' : 'Strategy' }}</th><th class="px-3 py-2 text-left">{{ $isTr ? 'Tür' : 'Type' }}</th><th class="px-3 py-2 text-left">{{ $isTr ? 'Sağlık' : 'Health' }}</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Kampanya' : 'Campaigns' }}</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Hedef EBM' : 'Target CPA' }}</th><th class="px-4 py-2 text-right">{{ $isTr ? 'Hedef ROAS' : 'Target ROAS' }}</th></tr></thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">{{ $isTr ? 'Portföy teklif stratejisi verisi yok.' : 'No portfolio bidding strategy data.' }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <section>
        <div class="rounded-xl bg-blue-50 p-5 ring-1 ring-inset ring-blue-100 dark:bg-blue-500/10 dark:ring-blue-500/20">
            ...
            <div class="mt-3 space-y-2 text-xs leading-5 text-blue-800 dark:text-blue-200">
                <p>• {{ $isTr ? 'Dönüşüm başı maliyet Google Ads dönüşümlerine göre hesaplanır; nitelikli müşteri adayı veya satış maliyeti değildir.' : 'Provider CPA uses Google Ads conversions; it is not qualified-lead or sale CPA.' }}</p>
                <p>• {{ $isTr ? 'Hedef maliyet tanımlı değilse MOXDOP kıyas için yalnız hesabın ortalama dönüşüm maliyetini kullanır.' : 'Without a target CPA, MOXDOP uses account provider CPA only as a relative benchmark.' }}</p>
                ...
                @if (($plan['target_roas'] ?? null) !== null)<p>• {{ $isTr ? 'Planda hedef ROAS kayıtlı; dönüşüm değeri / gelir verisi hazır olmadan otomatik karar ölçütü olarak kullanılmaz.' : 'Plan Target ROAS is stored, but it is not used for automated decisions until canonical conversion-value/revenue evidence exists.' }}</p>@endif
            </div>
        </div>
    </section>
</div>
