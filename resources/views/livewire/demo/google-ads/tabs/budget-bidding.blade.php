...
<div class="space-y-4">
    ...
    <section class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
        ...
        <div class="overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-gray-50 text-xs text-gray-500 dark:bg-white/[0.02]"><tr><th class="px-4 py-2 text-left">{{ $isTr ? 'Kampanya' : 'Campaign' }}</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Harcama' : 'Spend' }}</th><th class="px-3 py-2 text-right">Conv.</th><th class="px-3 py-2 text-right">CPA</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Gösterim payı' : 'Impr. share' }}</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Bütçe kaybı' : 'Lost to budget' }}</th><th class="px-3 py-2 text-right">{{ $isTr ? 'Sıralama kaybı' : 'Lost to rank' }}</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            @forelse ($campaigns as $row)
                <tr>
                    ...
                    <td class="px-3 py-2 text-right tabular-nums">{{ is_numeric($row['conversions'] ?? null) ? number_format((float)$row['conversions'],2,',','.') : '—' }}</td>
                    <td class="px-3 py-2 text-right tabular-nums">{{ is_numeric($row['cpa'] ?? null) ? number_format((float)$row['cpa'],2,',','.').' '.$currency : '—' }}</td>
                    <td class="px-3 py-2 text-right tabular-nums">{{ is_numeric($row['search_impression_share'] ?? null) ? number_format((float)$row['search_impression_share']*100,1,',','.').'%' : '—' }}</td>
                    <td class="px-3 py-2 text-right tabular-nums">{{ is_numeric($row['search_lost_is_budget'] ?? null) ? number_format((float)$row['search_lost_is_budget']*100,1,',','.').'%' : '—' }}</td>
                    <td class="px-3 py-2 text-right tabular-nums">{{ is_numeric($row['search_lost_is_rank'] ?? null) ? number_format((float)$row['search_lost_is_rank']*100,1,',','.').'%' : '—' }}</td>
                </tr>
            @empty
    ...
    <div class="rounded-xl bg-blue-50 px-4 py-3 text-sm text-blue-800 ring-1 ring-inset ring-blue-100 dark:bg-blue-500/10 dark:text-blue-200 dark:ring-blue-500/20">
        {{ $isTr ? 'Not: Ajans plan bütçesi henüz ayrı bir kanonik veri olarak tanımlı değilse pacing hedefi uydurulmaz. Buradaki bütçe/teklif görünümü yalnız provider verisini gösterir.' : 'Note: if agency planned budget is not stored canonically, pacing targets are not fabricated. This view shows provider-backed budget/bidding facts only.' }}
    </div>
</div>
