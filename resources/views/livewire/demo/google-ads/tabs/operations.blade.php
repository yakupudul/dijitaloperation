    ...
    @if ($ops === 'findings')
        <x-ta.table>
            ...
            @foreach ($opsData['findings'] as $f)
                <tr>
                    <td class="px-4 py-2.5"><x-ta.badge :color="match($f['severity']) { 'critical', 'high' => 'error', 'medium' => 'warning', default => 'light' }" size="sm">{{ $f['severity'] }}</x-ta.badge></td>
    ...
    @else
        <ul class="space-y-2">
            @foreach ($opsData['outcomes'] as $o)
                <li class="flex flex-col gap-1 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] sm:flex-row sm:justify-between">
                    <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $o['task'] }}</p>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">{{ $o['note'] }}</p>
                    </div>
                    <span @class(['text-xs font-semibold', 'text-emerald-700 dark:text-emerald-400' => $o['state'] === 'Improvement observed', 'text-amber-700 dark:text-amber-400' => $o['state'] !== 'Improvement observed'])>{{ $o['state'] }}</span>
                </li>
            @endforeach
        </ul>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <ol class="mt-3 space-y-2 text-sm">
                @foreach ($opsData['decision_history'] as $event)
                    <li class="flex gap-3"><span class="w-14 shrink-0 text-xs text-gray-400">{{ $event['when'] }}</span><span>{{ $event['event'] }}</span></li>
                @endforeach
            </ol>
        </section>
    @endif
...
@if ($selectedFinding)
    <x-demo.gads-drawer :title="$selectedFinding['title']" :subtitle="$selectedFinding['category'] ?? null" :severity="$selectedFinding['severity'] ?? null">
        @foreach (['what' => 'What happened', 'why' => 'Why this matters', 'scope' => 'Scope', 'evidence' => 'Evidence', 'next' => 'Recommended next action'] as $key => $label)
            ...
            @endif
        @endforeach
        <p class="text-[11px] text-gray-400">Tasks are not auto-created. No Google Ads write actions.</p>
    </x-demo.gads-drawer>
@endif
