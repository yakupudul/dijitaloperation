@php
    ...
    // URL state can outlive older UI versions. Never trust an arbitrary `ops`
    // query value as a Blade array key; fall back to the canonical first pane.
    $activeOps = array_key_exists($ops, $labels) ? $ops : 'findings';
    $activeRows = collect($opsData[$activeOps] ?? []);
@endphp
...
<div class="space-y-4">
    ...
    <div class="inline-flex rounded-lg ring-1 ring-inset ring-gray-300 dark:ring-gray-700" role="tablist">
        @foreach ($labels as $key => $label)
            <button type="button" wire:click="setOps('{{ $key }}')" @class([
                ...
                'bg-gray-100 text-gray-900 dark:bg-white/10 dark:text-white' => $activeOps === $key,
                'text-gray-600 dark:text-gray-300' => $activeOps !== $key,
            ])>{{ $label }}</button>
        @endforeach
    </div>
    ...
    @if ($activeRows->isEmpty())
        <div class="rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-6 py-10 text-center dark:border-gray-700 dark:bg-white/[0.02]">
            <h3 class="font-semibold text-gray-900 dark:text-white">{{ $labels[$activeOps] }} · {{ $isTr ? 'henüz kayıt yok' : 'no records yet' }}</h3>
            <p class="mx-auto mt-2 max-w-2xl text-sm text-gray-500">
                @if ($activeOps === 'findings')
                ...
                @elseif ($activeOps === 'recommendations')
