...
<section id="google-recommendations" class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 px-4 py-4 dark:border-gray-800">
            <div>
                <div class="flex flex-wrap items-center gap-2">
                    <h3 class="font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Google önerileri' : 'Google recommendations' }}</h3>
                    <span class="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{{ $currentRecommendations->count() }}</span>
                    @if ($latestObservedDate)
                        <span class="text-xs text-gray-400">{{ $isTr ? 'Son gözlem' : 'Last observed' }} · {{ $latestObservedDate }}</span>
                    @endif
                </div>
                <p class="mt-1 max-w-3xl text-xs text-gray-500 dark:text-gray-400">
                    {{ $isTr
                        ? 'Google’ın kendi önerileri; MOXDOP bulgusu değildir ve otomatik uygulanmaz. Uygun olanları Bulgu → Öneri → İş zincirine taşıyın.'
                        : 'Google’s own suggestions; they are not MOXDOP findings and are never auto-applied. Move relevant ones into the Finding → Recommendation → Task chain.' }}
                </p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ url('/findings') }}" wire:navigate class="rounded-lg px-3 py-1.5 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:text-gray-300 dark:ring-gray-700 dark:hover:bg-white/[0.03]">{{ $isTr ? 'Bulgular' : 'Findings' }}</a>
                <a href="{{ url('/recommendations') }}" wire:navigate class="rounded-lg px-3 py-1.5 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:text-gray-300 dark:ring-gray-700 dark:hover:bg-white/[0.03]">{{ $isTr ? 'Öneriler' : 'Recommendations' }}</a>
                <a href="{{ url('/tasks') }}" wire:navigate class="rounded-lg px-3 py-1.5 text-xs font-semibold text-brand-600 ring-1 ring-inset ring-brand-200 hover:bg-brand-50 dark:text-brand-400 dark:ring-brand-800 dark:hover:bg-brand-500/10">{{ $isTr ? 'İşler' : 'Tasks' }}</a>
            </div>
        </div>
        ...
        <div class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            @empty
                <div class="px-6 py-10 text-center">
                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Güncel Google önerisi yok' : 'No current Google recommendations' }}</h4>
                    <p class="mx-auto mt-2 max-w-2xl text-sm text-gray-500 dark:text-gray-400">
                        {{ $isTr
                            ? 'Google şu an öneri sunmuyor olabilir ya da öneriler henüz çekilmemiş olabilir. MOXDOP burada öneri uydurmaz.'
                            : 'Google may have no current recommendations, or they have not been collected yet. MOXDOP does not fabricate suggestions here.' }}
                    </p>
                </div>
            @endforelse
        </div>
