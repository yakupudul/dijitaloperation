...
<div class="space-y-4">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-1 max-w-4xl text-sm text-gray-500 dark:text-gray-400">{{ $isTr ? 'Kullanıcı sorguları, hedeflenen anahtar kelimeler, negatifler ve analizler ayrı çalışma alanlarıdır.' : 'User queries, targeting keywords, negatives, and analysis are separate work areas.' }}</p>
        </div>
        <div class="flex flex-wrap gap-2 text-xs">
            <span class="rounded-full bg-blue-50 px-2.5 py-1 font-medium text-blue-700 ring-1 ring-inset ring-blue-200 dark:bg-blue-500/10 dark:text-blue-300 dark:ring-blue-500/20">{{ $isTr ? 'Google verisi' : 'Provider facts' }}</span>
    ...
    @if (in_array($search_sub, ['terms', 'keywords'], true))
        <div class="rounded-xl bg-white p-3 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-6">
                <input type="search" wire:model.live.debounce.400ms="search_query" placeholder="{{ $isTr ? 'Terim, anahtar kelime, kampanya veya reklam grubu ara…' : 'Search term, keyword, campaign or ad group…' }}" class="h-9 w-full rounded-lg border border-gray-300 bg-white px-3 text-xs text-gray-800 outline-none focus:border-brand-400 xl:col-span-2 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-200">
    ...
    @if ($search_sub === 'terms')
        ...
        <div class="rounded-xl bg-amber-50 px-4 py-3 ring-1 ring-inset ring-amber-200 dark:bg-amber-500/10 dark:ring-amber-500/20">
            <p class="text-sm font-semibold text-amber-900 dark:text-amber-100">{{ $isTr ? 'Az satır görmek her zaman eksik veri çekimi demek değildir' : 'A small row count does not always mean incomplete collection' }}</p>
            <p class="mt-1 text-xs leading-5 text-amber-800 dark:text-amber-200/90">{{ $isTr ? 'Google, gizlilik ve düşük hacim eşikleri nedeniyle arama terimleri raporunda tüm sorguları tek tek göstermez. MOXDOP gizlenen sorguları uydurmaz. Arama ağı harcaması ile gösterilen terimlerin harcaması arasındaki oran, sorunun eksik veri çekiminden mi yoksa Google gizliliğinden mi kaynaklandığını ayırmaya yardım eder.' : 'Google does not individually disclose every query because of privacy and low-volume thresholds. MOXDOP never fabricates hidden queries. Comparing Search-network spend with disclosed term spend helps distinguish collection gaps from provider privacy limits.' }}</p>
        ...
        <div class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <div class="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-800"><div><h3 class="font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Arama terimleri' : 'Search terms' }}</h3><p class="mt-1 text-xs text-gray-500">{{ $isTr ? 'Google’ın seçili dönem için açıkladığı gerçek kullanıcı sorguları.' : 'Actual user queries Google disclosed for the selected period.' }}</p></div><div class="flex items-center gap-3"><span class="text-xs text-gray-500">{{ number_format((int)($termRowsTotal ?? 0),0,',','.') }} {{ $isTr ? 'sonuç' : 'results' }}</span>@if (method_exists($this, 'exportSearchTermsCsv') && (int) ($termRowsTotal ?? 0) > 0)<button type="button" wire:click="exportSearchTermsCsv" class="rounded-lg px-3 py-1.5 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:text-gray-200 dark:ring-gray-700 dark:hover:bg-white/5">{{ __('operator_gads.export.csv') }}</button>@endif</div></div>
            <div class="overflow-x-auto"><table class="min-w-[1120px] w-full text-sm"><thead class="bg-gray-50 text-[11px] uppercase tracking-wide text-gray-400 dark:bg-white/[0.02]"><tr>
                ...
                @empty
                    <tr><td colspan="12" class="px-5 py-12 text-center"><p class="font-medium text-gray-700 dark:text-gray-200">{{ $isTr ? 'Bu dönemde Google’ın gösterdiği arama terimi yok.' : 'Google disclosed no Search Terms for this period.' }}</p><p class="mx-auto mt-1 max-w-2xl text-xs leading-5 text-gray-500">{{ $isTr ? 'Bu 0 arama anlamına gelmez. Arama ağında harcama varsa üstteki görünürlük kartı Google gizliliği ile veri çekimi sorununu ayırır.' : 'This does not mean zero searches. If Search-network spend exists, the visibility card above distinguishes provider privacy from collection gaps.' }}</p></td></tr>
                @endforelse
            </tbody></table></div>
    ...
    @elseif ($search_sub === 'keywords')
        ...
        @endphp
        <div class="grid grid-cols-2 gap-3 xl:grid-cols-4"><x-ta.metric-card :label="$isTr ? 'Anahtar kelime listesi' : 'Keyword inventory'" :value="number_format($allKeywords->count(),0,',','.')" /><x-ta.metric-card :label="$isTr ? 'Dönem performansı olan' : 'With period performance'" :value="number_format($withPerformance,0,',','.')" /><x-ta.metric-card :label="$isTr ? 'Yalnız envanter' : 'Inventory only'" :value="number_format($withoutPerformance,0,',','.')" /><x-ta.metric-card :label="$isTr ? 'Anahtar kelime harcaması' : 'Keyword spend'" :value="$withPerformance ? $money($keywordSpend) : '—'" /></div>
        <div class="rounded-xl bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-800 ring-1 ring-inset ring-blue-200 dark:bg-blue-500/10 dark:text-blue-200 dark:ring-blue-500/20">{{ $isTr ? 'Liste ile dönem performansı ayrıdır. Metriği çekilmemiş anahtar kelime listede kalır fakat performansı “—” görünür; 0 sayılmaz. Sonradan kaldırılmış anahtar kelime seçili dönemde performans üretmişse raporda korunur.' : 'Inventory and period performance are separate. A keyword stays listed if period metrics are missing, but metrics show “—”, not fake zero. Removed keywords remain in historical reporting when they had period activity.' }}</div>
        <div class="overflow-hidden rounded-xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            ...
            <div class="overflow-x-auto"><table class="min-w-[1220px] w-full text-sm"><thead class="bg-gray-50 text-[11px] uppercase text-gray-400 dark:bg-white/[0.02]"><tr><th class="px-4 py-2.5 text-left">Keyword</th><th class="px-3 py-2.5 text-left">Match</th><th class="px-3 py-2.5 text-left">{{ $isTr ? 'Durum' : 'Status' }}</th><th class="px-3 py-2.5 text-left">{{ $isTr ? 'Kampanya' : 'Campaign' }}</th><th class="px-3 py-2.5 text-left">{{ $isTr ? 'Reklam grubu' : 'Ad group' }}</th><th class="px-3 py-2.5 text-right">{{ $isTr ? 'Gösterim' : 'Impr.' }}</th><th class="px-3 py-2.5 text-right">{{ $isTr ? 'Tıklama' : 'Clicks' }}</th><th class="px-3 py-2.5 text-right">CTR</th><th class="px-3 py-2.5 text-right">Avg. CPC</th><th class="px-3 py-2.5 text-right">{{ $isTr ? 'Harcama' : 'Cost' }}</th><th class="px-3 py-2.5 text-right">{{ $isTr ? 'Dönüşüm' : 'Conv.' }}</th><th class="px-3 py-2.5 text-right">CVR</th><th class="px-4 py-2.5 text-right">CPA</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($keywordRows ?? [] as $row)
                    <tr class="hover:bg-gray-50/70 dark:hover:bg-white/[0.02]"><td class="px-4 py-2.5 font-medium text-gray-900 dark:text-white">{{ $row['keyword'] ?? '—' }} @if(!($row['period_activity'] ?? false))<span class="ml-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-normal text-gray-500 dark:bg-white/5">{{ $isTr ? 'envanter' : 'inventory' }}</span>@endif</td><td class="px-3 py-2.5 text-xs">{{ $row['match'] ?? '—' }}</td><td class="px-3 py-2.5 text-xs">{{ filled($row['status'] ?? null) ? \App\Services\GoogleAds\Support\GoogleAdsDisplayFormat::status((string) $row['status']) : '—' }}</td><td class="px-3 py-2.5 text-xs text-gray-600 dark:text-gray-300">{{ $row['campaign'] ?? '—' }}</td><td class="px-3 py-2.5 text-xs text-gray-600 dark:text-gray-300">{{ $row['ad_group'] ?? '—' }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $number($row['impressions'] ?? null) }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $number($row['clicks'] ?? null) }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $pct($row['ctr'] ?? null) }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $money($row['avg_cpc'] ?? null) }}</td><td class="px-3 py-2.5 text-right tabular-nums font-medium">{{ $money($row['spend'] ?? null) }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $number($row['leads'] ?? null,2) }}</td><td class="px-3 py-2.5 text-right tabular-nums">{{ $pct($row['cvr'] ?? null) }}</td><td class="px-4 py-2.5 text-right tabular-nums">{{ $money($row['cpa'] ?? null) }}</td></tr>
                @empty<tr><td colspan="13" class="px-5 py-12 text-center text-gray-500">{{ $isTr ? 'Filtrelerle eşleşen anahtar kelime yok.' : 'No keywords match the filters.' }}</td></tr>@endforelse
            </tbody></table></div>
    ...
    @elseif ($search_sub === 'negatives')
        <div class="rounded-xl bg-white px-4 py-3 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800"><p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Negatif anahtar kelime kapsamı' : 'Negative keyword coverage' }}</p><p class="mt-1 text-xs leading-5 text-gray-500">{{ $isTr ? 'Kampanya ve reklam grubu negatifleri yalnız burada gösterilir. Arama teriminden negatif aday üretmek ayrı bir uzman değerlendirmesidir; Google Ads’e otomatik yazım yapılmaz.' : 'Campaign and ad-group negatives live only here. Turning a search term into a negative remains a separate expert decision; nothing is automatically written to Google Ads.' }}</p></div>
