@php
    ...
    $datasetLabel = function (string $dataset) use ($isTr, $containsAny): string {
        ...
        return match (true) {
            ...
            default => $isTr ? 'Google Ads veri seti' : 'Google Ads dataset',
        };
    };
...
<div class="space-y-5">
    <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div>
            ...
            <p class="mt-1 max-w-4xl text-sm text-gray-500 dark:text-gray-400">{{ $isTr ? 'Bağlantı durumunu, hesap kimliğini, geçmiş veri kapsamını, son veri çekimini ve her veri setinin durumunu tek yerde kontrol edin.' : 'Audit connection state, account identity, historical coverage, collection freshness and materialization health for every data family in one place.' }}</p>
        </div>
    ...
    <section class="rounded-2xl p-4 ring-1 ring-inset {{ $overallClasses }}">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm opacity-90">{{ $isTr ? 'Bu bir performans puanı değildir; bağlantının ve veri çekimlerinin gerçek durumundan hesaplanır.' : 'This is not a performance score; it is derived from actual connection, materialization and collection state.' }}</p>
            </div>
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]"><p class="text-xs text-gray-400">{{ $isTr ? 'Son başarılı veri çekimi' : 'Latest collection record' }}</p><p class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">{{ $lastCollectedAt ?: '—' }}</p></div>
    ...
    @if ($issues->isNotEmpty())
        <section class="overflow-hidden rounded-2xl border border-amber-200 bg-amber-50/60 dark:border-amber-500/20 dark:bg-amber-500/5">
            <div class="border-b border-amber-200 px-4 py-3 dark:border-amber-500/20"><h3 class="font-semibold text-amber-950 dark:text-amber-100">{{ $isTr ? 'Veri güvenini etkileyen konular' : 'Items affecting data trust' }}</h3><p class="mt-1 text-xs text-amber-800/80 dark:text-amber-200/70">{{ $isTr ? 'Önce bu veri setlerini kontrol edin. MOXDOP eksik veya hatalı veri için metrik uydurmaz.' : 'Review these data families first. MOXDOP does not fabricate metrics for incomplete or failed datasets.' }}</p></div>
            <div class="grid gap-2 p-3 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($issues->take(9) as $row)
                    <div class="rounded-xl bg-white/80 p-3 ring-1 ring-inset ring-amber-100 dark:bg-black/10 dark:ring-amber-500/10">
                        ...
                        <p class="mt-2 text-xs leading-5 text-gray-500 dark:text-gray-400">@if($isFailed($row)){{ $isTr ? 'Son veri çekimi hatayla sonuçlandı.' : 'Latest materialization state indicates an error.' }}@elseif($row['partial'] ?? false){{ $isTr ? 'Veri seti kısmi; bazı günler eksik olabilir.' : 'Dataset is marked partial; coverage may be incomplete.' }}@else{{ $isTr ? 'Başarılı son toplama zamanı henüz kayıtlı değil.' : 'No successful collection timestamp is recorded yet.' }}@endif</p>
                    </div>
                @endforeach
            </div>
        </section>
    ...
    <div class="grid gap-4 xl:grid-cols-[1fr_1.35fr]">
        <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <dl class="mt-4 space-y-3 text-sm">
                ...
                <div class="flex items-center justify-between gap-3"><dt class="text-gray-500">{{ $isTr ? 'Toplam harcama (tüm zamanlar)' : 'Lifetime spend' }}</dt><dd class="font-semibold text-gray-900 dark:text-white">{{ is_numeric(data_get($history,'lifetime_spend')) ? number_format((float)data_get($history,'lifetime_spend'),2,',','.').' '.$currency : '—' }}</dd></div>
            </dl>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <p class="mt-1 text-xs text-gray-500">{{ $isTr ? 'Bu ekran bağlantıyı ve MOXDOP’a alınmış veriyi gösterir; Google Ads hesabında değişiklik yapmaz.' : 'This screen audits the connection and data materialized into MOXDOP; it does not mutate the Google Ads account.' }}</p>
            <details class="mt-4">
            <summary class="cursor-pointer text-xs font-semibold text-gray-600 dark:text-gray-300">{{ __('operator_gads.technical_details') }}</summary>
            <div class="mt-3 grid gap-3 md:grid-cols-2">
                <div class="rounded-xl bg-emerald-50 p-3 text-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-200"><p class="text-sm font-semibold">{{ $isTr ? 'Google verisi' : 'Provider truth' }}</p><p class="mt-1 text-xs leading-5 opacity-80">{{ $isTr ? 'Google Ads’ten çekilen performans, ayar ve değişiklik kayıtları MOXDOP veri havuzundan okunur.' : 'Performance, configuration and Change Event data collected from Google Ads API are read from the local Data Pool.' }}</p></div>
                <div class="rounded-xl bg-blue-50 p-3 text-blue-800 dark:bg-blue-500/10 dark:text-blue-200"><p class="text-sm font-semibold">{{ $isTr ? 'Sayfa açılırken Google’a istek gitmez' : 'No provider call during render' }}</p><p class="mt-1 text-xs leading-5 opacity-80">{{ $isTr ? 'Sayfayı açmak Google Ads’e sorgu veya yazma işlemi göndermez; yenileme ayrı toplama akışını başlatır.' : 'Opening the page sends no provider query or mutation; refresh starts the separate collection workflow.' }}</p></div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-[1050px] w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-12 text-center text-sm text-gray-400">{{ $isTr ? 'Bu Google Ads hesabı için henüz veri çekimi kaydı yok.' : 'No Data Pool materialization records are available for this Google Ads asset yet.' }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <details class="rounded-xl bg-blue-50 px-4 py-3 text-sm text-blue-800 ring-1 ring-inset ring-blue-100 dark:bg-blue-500/10 dark:text-blue-200 dark:ring-blue-500/20">
        <summary class="cursor-pointer text-xs font-semibold text-gray-600 dark:text-gray-300">{{ __('operator_gads.technical_details') }}</summary>
        <p class="mt-2"><strong>{{ $isTr ? 'Veri çekme politikası:' : 'Collection policy:' }}</strong>
        {{ $isTr ? 'İlk bağlantıda hesabın tüm geçmişi aylık düzeyde taranır; aktif dönemler Google’ın izin verdiği aralıkta günlük ayrıntıyla geriye dönük çekilir. Normal güncellemelerde son dönem yeniden doğrulanır. Değişiklik geçmişi Google’ın saklama süresiyle sınırlıdır.' : 'At first connection, lifetime activity is discovered monthly; active periods are backfilled in detail inside supported granular windows. Normal updates restate the recent period. Change History remains subject to provider history limits.' }}</p>
    </details>
</div>
