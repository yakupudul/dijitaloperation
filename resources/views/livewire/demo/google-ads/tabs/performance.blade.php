...
<div class="space-y-5">
        ...
        <section class="rounded-2xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">{{ $isTr ? 'Google verisinden türetilen, otomatik işlem yapmayan öncelik sinyalleri.' : 'Priority signals derived from provider facts; no automatic changes are applied.' }}</p>
                </div>
            ...
            <div class="mt-4 space-y-4">
                ...
                <div class="rounded-xl border border-white/10 p-3">
                    @if ($hasValueTracking)
                        ...
                        <p class="mt-1 text-xs text-white/50">{{ $isTr ? 'Dönüşüm değeri Google’dan geliyor.' : 'Provider conversion value is available.' }}</p>
                    @else
    ...
    <section class="overflow-hidden rounded-2xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
        ...
        <div class="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            @empty
                <div class="col-span-full px-4 py-8 text-center text-sm text-gray-400">{{ $isTr ? 'Cihaz kırılımı için kullanılabilir Google verisi yok.' : 'No usable provider data for device performance.' }}</div>
            @endforelse
        </div>
    </section>
    ...
    <section class="rounded-2xl bg-white ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
        ...
        <div class="grid gap-4 p-4 xl:grid-cols-2">
            ...
            <div>
                <div class="mb-3"><h4 class="text-sm font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'İncelenecek saatler' : 'Hours to review' }}</h4><p class="mt-0.5 text-xs text-gray-500">{{ $isTr ? 'Harcama yapmış fakat Google Ads dönüşümü getirmemiş en pahalı zaman dilimleri. Otomatik kapatma önerisi değildir.' : 'Highest-spend windows with no provider conversions. This is not an automatic pause recommendation.' }}</p></div>
                <div class="space-y-2">
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        ...
        <section class="rounded-2xl bg-white p-4 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
            <div class="flex items-start justify-between gap-3"><div><h3 class="font-semibold text-gray-900 dark:text-white">{{ $isTr ? 'Kitle & demografi' : 'Audience & demographics' }}</h3><p class="mt-1 text-xs leading-5 text-gray-500">{{ $isTr ? 'Yalnız Google anlaşılır kitle etiketleri verdiğinde performans kararı üretir.' : 'Produces performance decisions only when provider labels are interpretable.' }}</p></div></div>
