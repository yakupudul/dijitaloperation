...
| Meta production collection (contract-driven) | YES | YES | NO | YES | YES | **PARTIAL** | Unmerged stacked child of PR #200; live Marketing API warehouse UAT not claimed. Google Ads keyword grain remains `IMPLEMENTED_UNPROVEN` on #200 and is not this slice. GBP remains isolated. No Meta specialist UX redesign. | Prompt 24 `MetaAdsDatasetExecutor` + Prompt 25 initial backfill + Prompt 27 incremental via shared Collection Engine / Data Pool; central Meta Integration + explicit `META_AD_ACCOUNT` bindings; incremental due selection uses exact preflight `core_asset_binding_ids` so sibling Meta Ads Digital Assets are not starved; other Meta integrations excluded |
...
Contract-driven Meta Ads collection already uses the shared Collection Engine and Data Pool (`MetaAdsDatasetExecutor`, `MetaInitialBackfillOrchestrator`, `MetaIncrementalCollectionOrchestrator`). This stacked child proves incremental due selection for exact preflight binding IDs across sibling Meta Ads Digital Assets, plus isolation from another Meta Integration.

**Not claimed:** live Marketing API / warehouse UAT, professional Meta Expert Workspace, Google Ads keyword-grain staging proof (still `IMPLEMENTED_UNPROVEN` on PR #200), or GBP.
