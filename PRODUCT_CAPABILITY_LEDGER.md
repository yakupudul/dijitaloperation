...
| Website production crawl collection | YES | YES | NO | NO | YES | **PARTIAL** | Live Moximu Website crawl / staging Collection Engine recollection remains **EXTERNAL UAT REQUIRED**. URL discovery is now provenance-based: site root, real same-site links, robots-declared sitemap URLs, and bounded sitemap fallbacks only; guessed paths such as `/contact-us`, `/location`, and `/locations` are not seeded. Sitemap indexes recurse into child URL sets while XML sitemap documents remain HTTP evidence and never become page inventory. HTTP failures, external redirect targets, WordPress/application error templates, and soft-404 pages retain HTTP/issue evidence but are excluded from `website_url` and page-level snapshots. Historical polluted warehouse rows are preserved for provenance but the operator console scopes counts, health, and previews to the latest `last_collection_run_id`. Active Website runs poll every 2 seconds and stop polling at a terminal state; technical storage fields stay in a collapsed details section. WordPress `WEB_RF_WP_REST` remains DEFERRED. | Shared Collection Engine `WebsiteDatasetExecutor` for COLLECTION_READY families: `WEB_RF_HTTP_HTML_DIAGNOSIS`, `WEB_RF_PUBLIC_CRAWL`, `WEB_RF_DNS_TLS`, `WEB_RF_PAGESPEED`. Deterministic URL batch identity keeps retry/resume idempotent; final-page relative resolution, aggregate byte limits, checkpoint-frozen `observed_at`, and per-tick progress deltas remain enforced. PHPUnit covers recursive sitemap inventory, soft-error exclusion, latest-run projection, and scoped live polling. Not DONE until live Moximu recollection and operator UAT pass. |
