...
| Global Service Catalog + Brand Service Areas | YES (branch) | NO | NO | YES | N/A | **CODE COMPLETE / UAT REQUIRED** | Tests intentionally not run. City/district values are operator-entered in this slice; a maintained geography reference import remains later. | Stable agency-wide Service IDs + aliases link to existing Brand Offering IDs. Brand form captures services, explicit priority and multiple country/city/district areas without creating service × area jobs. Structured Brand Context receives compatibility projections. |
