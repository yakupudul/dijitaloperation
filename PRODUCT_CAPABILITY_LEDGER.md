...
- Meta Ads: bound `meta_campaign_daily` + `meta_campaign_snapshot` → existing inactive-campaign-with-spend rule; daily window uses non-partial verified contiguous coverage, and snapshot status is taken only from a completed usable `meta_campaign_snapshot` DatasetRun/materialization (running/failed/partial entity snapshots are never `response_ok=true`)
