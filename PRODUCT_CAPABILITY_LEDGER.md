...
| Search Query Library | YES (branch) | NO | NO | YES | NO | **CODE COMPLETE / UAT REQUIRED** | Tests intentionally not run. Import runs synchronously and is bounded by upload/row limits. AI classification, Brand Query Portfolio, SERP validation and URL ownership are later phases. | Manual, pasted, CSV/TSV/TXT/XLSX queries with TR-safe normalization, service association, market/language, demand-family placeholder, branded exclusion state, source provenance and optional Ads/GSC/DataForSEO metrics. Missing metrics stay missing. |
