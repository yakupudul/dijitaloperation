...
| Async execution | YES | YES | NO | YES | YES | TESTED | Cancellation not implemented (future); cross-asset packs still sync; resource discovery refresh still sync | `AsyncOperationService` + Activity Center + database queue; see `OPERATOR_ASYNC_EXECUTION.md` |
