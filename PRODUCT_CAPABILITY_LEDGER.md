...
| Agency brain / operational synthesis (Phase C.1) | YES | YES | NO | N/A | N/A | **PARTIAL** | Canonical child is PR #206 (PR #205 superseded). Live provider/staging UAT from #200/#203/#204 remains an isolated external gate and is **not** claimed here. No BrainV2 / FindingV2 / Result entity / auto-Task / Agency Learning. Document Head charset/viewport/OG stay unevaluated when those snapshot fields were not collected. Document Head uses the proven homepage (`primary_url` or `website_http_snapshot` requested→final redirect target) and fails closed (`unproven_website_homepage_snapshot`) instead of selecting an arbitrary same-`observed_at` sibling page. Meta primary-result rules stay unevaluated without a collected `primary_result.status`. Google/Meta collected-facts adapters synthesize a 28-day window only from non-partial verified contiguous DatasetMaterialization coverage; rows last written by completed DatasetRuns in that window are included after incremental refresh, and running/failed partial batches are never `response_ok=true`. Meta inactive-campaign synthesis uses only the latest successful `meta_campaign_snapshot` DatasetRun; stale entities from older completed refreshes and later-failed entity snapshot rows cannot open or resolve Findings. | `EvaluateFindingsForAssetJob` runs canonical `FindingEvaluationService` then `CollectedFactsAnalysisService`. Website `DocumentHeadEvaluator` (`website_metadata_snapshot`); Google Ads `GoogleAdsPerformanceBoundEvidenceEvaluator` (`google_ads_campaign_daily` spend-with-zero-conversions); Meta Ads `MetaAdsPerformanceBoundEvidenceEvaluator` (`meta_campaign_daily` + snapshot inactive-with-spend). `FindingEvaluationService` now emits `FindingEvaluationCompleted` so Outcome V1 observes GSC/GA4 canonical evaluations. Manual `CreateTaskFromRecommendation`. Sibling Brand/provider warehouse rows never enter collected-facts runs. Not DONE. |
...
- Website/SEO: `website_metadata_snapshot` → existing Document Head rules (only collected dimensions) on the proven homepage URL (`primary_url` slash variants or the HTTP snapshot redirect target of that request). Multi-page crawls with a shared checkpoint `observed_at` never fall back to a higher-ID sibling such as `/contact`; unproven homepages skip as `unproven_website_homepage_snapshot` and do not open or resolve Findings
- Google Ads: bound `google_ads_campaign_daily` → existing campaign spend-with-zero-conversions rule, only from a non-partial verified contiguous DatasetMaterialization window; completed DatasetRun rows across incremental refreshes are included, running/failed partial batches are not synthesized
- Meta Ads: bound `meta_campaign_daily` + `meta_campaign_snapshot` → existing inactive-campaign-with-spend rule; daily window uses non-partial verified contiguous coverage across completed DatasetRuns. Snapshot status is taken only from the materialization's latest successful `meta_campaign_snapshot` DatasetRun (stale entities from older completed refreshes, and running/failed/partial entity snapshots, are never current / `response_ok=true`)
