...
/**
 * Resolves Anthropic API credentials: DB provider credential → ANTHROPIC_API_KEY → missing.
 */
class AnthropicCredentialResolver
{
    ...
    {
        $value = config('moxdop.anthropic.api_key');

        if (is_string($value) && filled($value)) {
            return $value;
        }

        $legacy = config('ai.providers.anthropic.key');
