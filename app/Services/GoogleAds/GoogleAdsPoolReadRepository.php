 ...
 *
 * Resource-first collection is canonical. During migration an account may still have
 * older asset-bound rows, so every read chooses exactly one source mode: central
 * (digital_asset_id NULL) when it exists for the requested scope, otherwise the
 * legacy asset-bound rows. Sources are never summed together.
 */
class GoogleAdsPoolReadRepository
{
    ...
    public function accountDailySums(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end): array
    {
        $row = $this->dailyScope('google_ads_account_daily', $digitalAssetId, $externalResourceId, $customerId, $start, $end)
    ...
    public function accountDailySeries(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end): array
    {
        return $this->dailyScope('google_ads_account_daily', $digitalAssetId, $externalResourceId, $customerId, $start, $end)
    ...
    public function campaignPerformance(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end, int $limit = 100): array
    {
        $useCentral = $this->centralExists('google_ads_campaign_daily', $externalResourceId, $customerId, $start, $end);
        $perf = $this->scopeWithMode('google_ads_campaign_daily', $digitalAssetId, $externalResourceId, $customerId, $useCentral)
        ...
        $snapshots = [];
        if ($campaignIds !== []) {
            $snapshots = $this->scopeWithMode('google_ads_campaign_snapshot', $digitalAssetId, $externalResourceId, $customerId, $useCentral)
                ->whereIn('campaign_id', $campaignIds)
                ->get(['campaign_id', 'metadata'])
                ->keyBy('campaign_id')->all();
        }

        $budgets = $this->scopeWithMode('google_ads_campaign_budget_snapshot', $digitalAssetId, $externalResourceId, $customerId, $useCentral)
            ->get(['budget_id', 'metadata'])->keyBy('budget_id');
        ...
        foreach ($perf as $row) {
            ...
            $budgetMeta = $budgetId !== null && isset($budgets[$budgetId]) ? $this->decodeMetadata($budgets[$budgetId]->metadata) : [];
            $rows[] = [
                ...
                'lost_is_budget' => $this->metadataFloat($meta, 'search_budget_lost_impression_share'),
                'lost_is_rank' => $this->metadataFloat($meta, 'search_rank_lost_impression_share'),
    ...
    /** @return list<array<string,mixed>> */
    public function topKeywords(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end, int $limit = 100): array
    {
        $useCentral = $this->centralExists('google_ads_keyword_daily', $externalResourceId, $customerId, $start, $end);
        $perf = $this->scopeWithMode('google_ads_keyword_daily', $digitalAssetId, $externalResourceId, $customerId, $useCentral)
            ->whereBetween('reporting_date', [$start, $end])
            ->groupBy('ad_group_id', 'criterion_id')->orderByDesc(DB::raw('SUM(cost_amount)'))->limit($limit)
            ->get([
                'ad_group_id', 'criterion_id', DB::raw('SUM(impressions) as impressions'), DB::raw('SUM(clicks) as clicks'),
                DB::raw('SUM(cost_amount) as cost_amount'), DB::raw('SUM(conversions) as conversions'), DB::raw('MAX(currency) as currency'),
            ]);

        $snapshots = [];
        if ($perf->isNotEmpty()) {
            $snapshots = $this->scopeWithMode('google_ads_keyword_snapshot', $digitalAssetId, $externalResourceId, $customerId, $useCentral)
                ->where(function ($query) use ($perf): void {
                    foreach ($perf as $row) {
                        $query->orWhere(function ($inner) use ($row): void {
                            $inner->where('ad_group_id', $row->ad_group_id)->where('criterion_id', $row->criterion_id);
                        });
                    }
                })->get(['ad_group_id', 'criterion_id', 'metadata'])
                ->keyBy(fn ($row): string => (string) $row->ad_group_id."\0".(string) $row->criterion_id)->all();
        }
    ...
    /** @return list<array<string,mixed>> */
    public function topLandingPages(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end, int $limit = 100): array
    {
        return $this->dailyScope('google_ads_landing_page_daily', $digitalAssetId, $externalResourceId, $customerId, $start, $end)
            ->groupBy('landing_page')->orderByDesc(DB::raw('SUM(cost_amount)'))->limit($limit)
            ->get([
                'landing_page', DB::raw('SUM(impressions) as impressions'), DB::raw('SUM(clicks) as clicks'),
                DB::raw('SUM(cost_amount) as cost_amount'), DB::raw('SUM(conversions) as conversions'), DB::raw('MAX(currency) as currency'),
            ])
            ->map(static fn ($row): array => [
    ...
    /** @return list<array<string,mixed>> */
    public function conversionActions(int $digitalAssetId, int $externalResourceId, string $customerId): array
    {
        return $this->snapshotScope('google_ads_conversion_action_snapshot', $digitalAssetId, $externalResourceId, $customerId)
    ...
    /** @return list<array<string,mixed>> */
    public function conversionActionDailySums(int $digitalAssetId, int $externalResourceId, string $customerId, string $start, string $end): array
    {
        return $this->dailyScope('google_ads_conversion_action_daily', $digitalAssetId, $externalResourceId, $customerId, $start, $end)
            ->groupBy('conversion_action_id')
            ->get([
                'conversion_action_id', DB::raw('SUM(conversions) as conversions'),
                DB::raw('SUM(all_conversions) as all_conversions'), DB::raw('SUM(conversions_value) as conversions_value'),
            ])
            ->map(static fn ($row): array => [
                'conversion_action_id' => (string) $row->conversion_action_id,
                'conversions' => (float) $row->conversions,
    ...
    /** @return list<array<string,mixed>> */
    public function adSnapshots(int $digitalAssetId, string $customerId, int $limit = 50, ?int $externalResourceId = null): array
    {
        $query = $externalResourceId !== null
            ? $this->snapshotScope('google_ads_ad_snapshot', $digitalAssetId, $externalResourceId, $customerId)
            : DB::table('google_ads_ad_snapshot')->where('digital_asset_id', $digitalAssetId)->where('customer_id', $customerId);
    ...
    /** @return list<array<string,mixed>> */
    public function assetCoverage(int $digitalAssetId, string $customerId, int $limit = 50, ?int $externalResourceId = null): array
    {
        $query = $externalResourceId !== null
            ? $this->snapshotScope('google_ads_asset_coverage_snapshot', $digitalAssetId, $externalResourceId, $customerId)
            : DB::table('google_ads_asset_coverage_snapshot')->where('digital_asset_id', $digitalAssetId)->where('customer_id', $customerId);
    ...
    {
        return $this->scopeWithMode($table, $digitalAssetId, $externalResourceId, $customerId, $this->centralExists($table, $externalResourceId, $customerId));
    }

    private function centralExists(string $table, int $externalResourceId, string $customerId, ?string $start = null, ?string $end = null): bool
    {
    ...
    private function scopeWithMode(string $table, int $digitalAssetId, int $externalResourceId, string $customerId, bool $central): Builder
    {
        $query = DB::table($table)
            ->where('external_resource_id', $externalResourceId)
            ->where('customer_id', $customerId);
