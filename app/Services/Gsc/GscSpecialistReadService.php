...
final class GscSpecialistReadService
{
    ...
    private const array PROVENANCE_FIELDS = [
        ...
        'glance.impressions',
        'glance.ctr',
        'glance.search_attention',
        'performance_trend.clicks',
        'performance_trend.impressions',
        'search_momentum',
        'page_pulse',
        'discoverability',
        'performance.devices',
        'performance.countries',
        'performance.brand_nonbrand',
    ...
    private function buildRealWorkspace(
        ...
        $data['glance'] = $this->realGlance($propertyGate, $sums, $prevSums, $data['glance']);
        ...
        $provenance['indexing.urls'] = $inspectionGate->isUsable()
            ? DataSourceState::ProviderLimited->value
            : DataSourceState::Unavailable->value;
    ...
    private function operationalWorkspace(
        ...
        $reason = $errorMessage !== null
            ? 'query_error'
            : ($binding->reason ?? 'no_active_search_console_binding');
        ...
        $data = [
            ...
            'freshness' => [],
            'glance' => [
                'clicks' => $unavailableChip,
                'impressions' => $unavailableChip,
                'ctr' => $unavailableChip,
                'search_attention' => $unavailableChip,
            ],
            'needs_attention' => [],
            'performance_trend' => [
                'labels' => [],
            ...
            'search_momentum' => ['note' => 'Unavailable — '.$reason],
            'page_pulse' => [],
            ...
            'performance' => [
                'devices' => [],
                'countries' => [],
                'brand_nonbrand' => ['source' => 'Unavailable', 'note' => 'Unavailable — '.$reason],
                'diagnosis' => ['interpretation' => 'Unavailable — '.$reason],
            ],
            'demand' => [
                'clusters' => [],
                'queries' => [],
                'momentum' => [],
                'ownership_reviews' => [],
                'observed_query_note' => GscWorkspaceFixtures::demand(GscWorkspaceFixtures::aggregateProperty($rangeStart, $rangeEnd))['observed_query_note'],
            ],
            'pages' => [
                'subtitle' => 'Unavailable — '.$reason,
                'directory' => [],
                'attribution_note' => 'GA4 context unavailable — not connected.',
            ],
            'indexing' => [
                'subtitle' => 'Unavailable — '.$reason,
                'coverage' => ['state' => 'Unavailable', 'note' => 'Site-wide indexing totals unavailable from GSC API.'],
                'urls' => [],
                'sitemaps' => [],
                'reconciliation' => ['note' => 'Unavailable — '.$reason],
                'inspection_note' => 'URL Inspection is a selective sample only — never extrapolated to uninspected URLs.',
            ],
            'relationships' => [
                'observes' => [],
                'provides_evidence_to' => [],
                'technical_connection' => [
                    'type' => 'Search Console property binding',
                    'property' => null,
                    'property_type' => null,
                    'status' => $statusLabel,
                    'note' => $errorMessage !== null
                        ? 'Read error — retry once the underlying issue is resolved.'
                        : 'Connect a Search Console property to this Digital Asset to enable real data.',
                ],
            ],
            'operations' => [
                'subtitle' => $errorMessage !== null
                    ? 'Search Console read error — no findings, recommendations, tasks, or outcomes available.'
                    : 'Search Console binding required — connect a property to see findings, recommendations, tasks, and outcomes.',
                'findings' => [],
                'recommendations' => [],
                'tasks' => [],
                'outcomes' => [],
    ...
    private function realIdentity(GscBindingContext $binding, ?DigitalAsset $asset): array
    {
        ...
        $propertyType = str_starts_with((string) $binding->siteUrl, 'sc-domain:')
            ? 'Domain property'
            : 'URL-prefix property';

        return [
            'eyebrow' => 'Google Search Console',
            'title' => "{$title} — Search Console",
            'brand' => $brandName,
            'brand_id' => $asset?->brand_id,
            ...
            'gbp_asset_id' => null,
            'gsc_asset_id' => $binding->assetId,
            'relationship_line' => $assetName !== null ? "Observes · {$assetName}" : null,
            'property_label' => $binding->siteUrl,
    ...
    /**
     * @param  array{clicks?: int, impressions?: int, position_weighted_numerator?: float, position_impressions?: int}|null  $sums
     * @param  array{clicks?: int, impressions?: int, position_weighted_numerator?: float, position_impressions?: int}|null  $prevSums
     * @param  array<string, mixed>  $demoGlance
     * @return array<string, mixed>
     */
    private function realGlance(
        GscDatasetReadiness $propertyGate,
        ?array $sums,
        ?array $prevSums,
        array $demoGlance,
    ): array {
        ...
        $ctrResult = $sums !== null
            ? $this->formulas->ctr($clicksRaw, $impressionsRaw)
            : FormulaResult::state(FormulaResult::STATE_NOT_COLLECTED);

        $positionResult = ($sums !== null && ($sums['position_impressions'] ?? 0) > 0)
            ? $this->formulas->impressionWeightedPosition(
                (float) $sums['position_weighted_numerator'],
                (int) $sums['position_impressions'],
            )
            : FormulaResult::state(FormulaResult::STATE_NOT_COLLECTED);

        $clicksDelta = ($sums !== null && $prevSums !== null)
            ? $this->formulas->periodRelativeChange((float) $sums['clicks'], (float) $prevSums['clicks'])
            : null;
        $impressionsDelta = ($sums !== null && $prevSums !== null)
            ? $this->formulas->periodRelativeChange((float) $sums['impressions'], (float) $prevSums['impressions'])
            : null;

        $prevCtr = ($prevSums !== null)
            ? $this->formulas->ctr((int) $prevSums['clicks'], (int) $prevSums['impressions'])
            : null;
        ...
        $ctr = [
            'value' => $ctrResult->isValue()
                ? number_format((float) $ctrResult->toPercentDisplay(), 2).'%'
                : '—',
            'raw' => $ctrResult->isValue() ? (float) $ctrResult->toDisplay() : null,
            'secondary' => $ctrDeltaPp !== null
                ? (($ctrDeltaPp > 0 ? '+' : '').$ctrDeltaPp.'pp vs previous · sum(clicks)/sum(impressions)')
                : 'vs previous period unavailable',
            'tone' => 'neutral',
        ];
    ...
    private function deltaSecondary(?FormulaResult $delta, GscDatasetReadiness $gate): string
    {
        if (! $gate->isUsable()) {
    ...
    private function realPerformanceTrend(
        ...
        if (! $propertyGate->isUsable() || $propertyGate->effectiveStart === null || $propertyGate->effectiveEnd === null) {
            return [
                ...
                'clicks' => [],
                'impressions' => [],
                'note' => 'Performance trend unavailable — gsc_property_daily dataset is not ready for real UI.',
            ];
        }

        $series = $this->pool->propertyDailySeries(
            $digitalAssetId,
            $externalResourceId,
            $siteUrl,
            $propertyGate->effectiveStart,
            $propertyGate->effectiveEnd,
        );
        ...
        return [
            ...
            'clicks' => $clicks,
            'impressions' => $impressions,
            'note' => ($partial ? 'Real GSC property daily data (partial coverage). ' : 'Real GSC property daily data. ')
                .self::POSITION_PROVENANCE_NOTE,
        ];
    }
     ...
     */
    private function realMetricSeries(
        GscDatasetReadiness $propertyGate,
        int $digitalAssetId,
        int $externalResourceId,
        string $siteUrl,
        ...
        foreach ($series as $point) {
            ...
            $ctr[] = $dayCtr->isValue() ? round((float) $dayCtr->toDisplay(), 4) : 0.0;

            $dayPosition = ($point['position'] !== null && $point['impressions'] > 0)
                ? (float) $point['position']
                : null;
            $position[] = $dayPosition ?? 0.0;
        }
    ...
    /**
     * @param  array<string, mixed>  $demoPerformance
     * @return array<string, mixed>
     */
    private function realPerformance(
        int $digitalAssetId,
        int $externalResourceId,
        string $siteUrl,
        GscDatasetReadiness $deviceGate,
        GscDatasetReadiness $countryGate,
        array $demoPerformance,
    ): array {
        $devices = [];
        if ($deviceGate->isUsable() && $deviceGate->effectiveStart !== null && $deviceGate->effectiveEnd !== null) {
            $rows = $this->pool->devices(
                $digitalAssetId,
                $externalResourceId,
                $siteUrl,
                $deviceGate->effectiveStart,
                $deviceGate->effectiveEnd,
            );
            foreach ($rows as $row) {
                $ctr = $this->formulas->ctr($row['clicks'], $row['impressions']);
                $position = ($row['position_impressions'] ?? 0) > 0
                    ? $this->formulas->impressionWeightedPosition(
                        (float) $row['position_weighted_numerator'],
                        (int) $row['position_impressions'],
                    )
                    : FormulaResult::state(FormulaResult::STATE_NOT_COLLECTED);

                $devices[] = [
                    'device' => $this->humanizeDevice((string) $row['device']),
                    'clicks' => $row['clicks'],
                    'impressions' => $row['impressions'],
                    'ctr' => $ctr->isValue() ? round((float) $ctr->toPercentDisplay(), 1) : null,
                    'position' => $position->isValue() ? round((float) $position->toDisplay(), 1) : null,
                    'position_note' => self::POSITION_PROVENANCE_NOTE,
                ];
            }
        }
        ...
        if ($countryGate->isUsable() && $countryGate->effectiveStart !== null && $countryGate->effectiveEnd !== null) {
            $rows = $this->pool->countries(
                $digitalAssetId,
                $externalResourceId,
                $siteUrl,
                $countryGate->effectiveStart,
                $countryGate->effectiveEnd,
            );
            foreach ($rows as $row) {
                $countries[] = [
                    ...
                    'clicks' => $row['clicks'],
                    'impressions' => $row['impressions'],
                    'note' => 'Impressions are observed appearances — not search volume.',
                ];
            }
        }
    ...
    /**
     * @param  array<string, mixed>  $demoDemand
     * @return array<string, mixed>
     */
    private function realDemand(
        int $digitalAssetId,
        int $externalResourceId,
        string $siteUrl,
        GscDatasetReadiness $queryGate,
        array $demoDemand,
    ): array {
        $queries = [];
        if ($queryGate->isUsable() && $queryGate->effectiveStart !== null && $queryGate->effectiveEnd !== null) {
            $rows = $this->pool->topQueries(
                $digitalAssetId,
                $externalResourceId,
                $siteUrl,
                $queryGate->effectiveStart,
                $queryGate->effectiveEnd,
            );
            foreach ($rows as $row) {
                $ctr = $this->formulas->ctr($row['clicks'], $row['impressions']);
                $position = ($row['position_impressions'] ?? 0) > 0
     ...
     * @return list<array<string, mixed>>
     */
    private function realPagesDirectory(
        int $digitalAssetId,
        int $externalResourceId,
        string $siteUrl,
        GscDatasetReadiness $pageGate,
    ): array {
        if (! $pageGate->isUsable() || $pageGate->effectiveStart === null || $pageGate->effectiveEnd === null) {
            return [];
        }

        $rows = $this->pool->topPages(
            $digitalAssetId,
            $externalResourceId,
            $siteUrl,
            $pageGate->effectiveStart,
            $pageGate->effectiveEnd,
        );
        ...
        return array_map(static fn (array $row): array => [
            ...
            'ga4_context' => [
                ...
                'mapped_actions' => null,
                'note' => 'GA4 page context unavailable on real path — not query-attributed.',
            ],
            'website_attention' => null,
        ], $rows);
    }

    /**
     * @param  array<string, mixed>  $demoIndexing
     * @return array<string, mixed>
     */
    private function realIndexing(
        int $digitalAssetId,
        string $siteUrl,
        GscDatasetReadiness $sitemapGate,
        GscDatasetReadiness $inspectionGate,
        array $demoIndexing,
    ): array {
        $sitemaps = [];
        if ($sitemapGate->isUsable()) {
            foreach ($this->pool->sitemaps($digitalAssetId, $siteUrl) as $row) {
                ...
                $sitemaps[] = [
                    'path' => $row['path'],
                    'submitted' => $meta['last_submitted'] ?? null,
                    'last_downloaded' => $meta['last_downloaded'] ?? null,
                    'discovered' => $submittedTotal > 0 ? $submittedTotal : null,
                    'warnings' => $meta['warnings'] ?? 0,
                    'errors' => $meta['errors'] ?? 0,
                    'status' => ($meta['errors'] ?? 0) > 0 ? 'Errors' : 'Success',
                    'note' => 'Submitted URL count ≠ indexed URL count — deprecated indexed field is never used.',
                ];
            }
        }
        ...
        if ($inspectionGate->isUsable()) {
            foreach ($this->pool->urlInspectionSamples($digitalAssetId, $siteUrl) as $row) {
                ...
                $urls[] = [
                    'id' => 'url-'.Str::slug($row['page']),
                    'path' => $this->pathFromUrl($row['page']),
                    'url' => $row['page'],
                    'role' => '',
                    'sitemap' => is_array($meta['sitemap'] ?? null) && ($meta['sitemap'] ?? []) !== []
                        ? 'Present'
                        : 'Unknown',
                    'index_state' => $meta['coverage_state'] ?? 'Unavailable',
                    'last_crawl' => $meta['last_crawl_time'] ?? null,
                    'canonical' => ($userCanonical !== null && $googleCanonical !== null && $userCanonical !== $googleCanonical)
                        ? 'Mismatch'
                        : ($userCanonical !== null || $googleCanonical !== null ? 'Match' : 'Unknown'),
                    'user_canonical' => $userCanonical,
                    'google_canonical' => $googleCanonical,
                    'search_visibility' => 'Sample only',
                    'attention' => null,
                    'sample_note' => 'URL Inspection selective sample — not extrapolated to uninspected URLs.',
                ];
            }
        }

        return [
            'subtitle' => 'Google index state and sitemap metadata — read-only observation from pool snapshots.',
            'coverage' => [
                'indexed' => null,
                'not_indexed' => null,
                'unknown' => null,
                'excluded' => null,
                'state' => 'Unavailable',
                'note' => 'Site-wide Page Indexing totals are unavailable from the GSC API — not substituted with Demo numbers.',
            ],
            'urls' => $urls,
            'sitemaps' => $sitemaps,
            'reconciliation' => [
    ...
    {
        return [
            'subtitle' => 'Site-wide discoverability funnel unavailable on real path — GSC API cannot supply full index coverage totals.',
            'stages' => [],
            'note' => 'GA4 actions are page-attributed — GSC cannot prove query→conversion. Index observed site-wide totals remain unavailable.',
        ];
    }

    /**
     * @param  array<string, GscDatasetReadiness>  $gates
     * @return array<string, mixed>
     */
    private function realCollectionState(array $gates): array
    {
        return [
            'note' => 'Real GSC collection/materialization/freshness/integrity/coverage state. Findings, Recommendations, Tasks and Outcomes below remain Demo — this migration creates no Evidence/Findings/Opportunities.',
            'datasets' => array_map(static fn (GscDatasetReadiness $g): array => $g->toArray(), $gates),
        ];
    }
    ...
    private function humanizeDevice(string $device): string
    {
        return match (strtoupper($device)) {
            ...
            'DESKTOP' => 'Desktop',
            'TABLET' => 'Tablet',
            default => Str::title(strtolower($device)),
        };
    }
