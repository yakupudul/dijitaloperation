...
class CrossAssetWebsiteGbpAddressConsistencyActionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_action_is_visible_for_website_assets(): void
    {
        ...
        Livewire::test(ViewDigitalAsset::class, [
            ...
            ->assertOk()
            ->assertActionVisible('runWebsiteGbpAddressConsistency');
    }
    ...
    public function test_action_runs_pack_and_redirects_to_run(): void
    {
        ...
        Livewire::test(ViewDigitalAsset::class, [
            ...
            ->callAction('runWebsiteGbpAddressConsistency')
            ->assertHasNoActionErrors()
            ->assertRedirect();

        $run = Run::query()
            ->where('digital_asset_id', $website->id)
            ->where('module_id', CrossAssetWebsiteGbpAddressConsistencyService::MODULE_ID)
            ->where('metadata->pack_id', CrossAssetWebsiteGbpAddressConsistencyService::PACK_ID)
            ->latest('id')
            ->first();
