...
class OperatorAppAuthConvergenceTest extends TestCase
{
    ...
    {
        $this->get('/app/customers')->assertRedirect('/app/login');
    }
    ...
    public function test_system_login_redirects_to_app_login_without_filament_ui(): void
    {
        $this->get('/system/login')
            ->assertRedirect('/app/login');

        $this->followRedirects($this->get('/system/login'))
            ->assertOk()
            ->assertSee('MoxDOP')
            ->assertDontSee('filament', false);
    }

    public function test_system_root_redirects_to_app_without_legacy_dashboard(): void
    {
        $this->get('/system')->assertRedirect('/app');
    }
