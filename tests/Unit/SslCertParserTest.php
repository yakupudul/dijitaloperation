...
class SslCertParserTest extends TestCase
{
    ...
    public function test_fingerprint_is_deterministic_sha1_of_host_ssl_valid_to_issuer(): void
    {
        $parser = new SslCertParser;

        $expected = sha1('acme.example'.'ssl'.'2026-01-01T00:00:00Z'.'Test Intermediate CA');

        $this->assertSame(
            $expected,
            $parser->fingerprint('Acme.Example', '2026-01-01T00:00:00Z', 'Test Intermediate CA'),
        );
    }
}
