...
| keyword snapshot | Google Ads GAQL | yes, bound | yes | yes | yes | `google_ads_keyword_snapshot` | keyword snapshot | current | multi-snapshot | incremental via freshness planner | dataset 39; 735 rows under the previous customer×criterion_id grain after last-write-wins collapse. Grain is now customer×ad_group×criterion; staging was not re-collected after this correction | historical 735 is collapsed inventory, not the corrected grain | none on staging | `PROVEN_STAGING` |
