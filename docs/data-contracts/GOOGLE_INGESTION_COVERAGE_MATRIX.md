...
- Google Ads snapshot collection: `PROVEN_STAGING` except **keyword snapshot grain** (`customer × ad_group × criterion`) which is `IMPLEMENTED_UNPROVEN` until a current-code recollection; historical 735-row collapsed inventory is preserved separately
