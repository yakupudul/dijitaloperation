...
**Operator routes (ADR-044):** site root (`/`, `/login`, `/customers`, `/brands`, `/assets`, `/integrations`, `/tasks`, `/activity`, `/findings`, …). Filament technical/admin panel: `/admin`. Legacy `/app` and `/system` return HTTP 410 and are not live read paths.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Auth (web guard) | REAL | Laravel session auth | `/login` (operator); `/admin/login` (Filament) | User credentials | Feature auth tests | none | Legacy `/app` `/system` login → 410 | — |
...
| WordPress site connector | PARTIAL | Site connector services | `/integrations/site-connectors` | Pairing/probe | Connector tests | none | Full catalog sections deferred historically | Deploy + WP credentials |
...
| Capability | Status | Canonical source | Read path | Write path | Tests | Demo dependency | Gap | Blocker |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GA4 workspace (bound, pool KPIs) | REAL | `Ga4SpecialistReadService` + pool | `/assets/analytics/{id}` | Collectors only | GA4 migration tests | catalog-only for `ga4-atlas` | Users period sum UNAVAILABLE by rule | Binding + data |
...
| GSC workspace (bound KPIs) | REAL | `GscSpecialistReadService` | `/assets/search-console/{id}` | Collectors | GSC tests | catalog-only for `gsc-atlas` | Indexing totals / attention heuristics | Binding + data |
...
| Google Ads workspace (bound KPIs) | REAL | `GoogleAdsSpecialistReadService` | `/assets/google-ads/{id}` | Collectors | Ads tests | catalog-only for `gads-atlas` | Search clusters/inbox | Binding + data |
...
| Meta Ads workspace (bound KPIs) | REAL | `MetaAdsSpecialistReadService` | `/assets/meta/{id}` | Collectors | Meta tests | catalog-only for `meta-atlas` | Reach/frequency/result mix UNAVAILABLE by contract | Binding + data |
...
| Website specialist analytics (production) | UNAVAILABLE | `UnavailableWorkspaceShells::website` | `/assets/website/{id}` Overview | Observations not wired to the operator shell | `DemoRealityFinalConvergenceTest` | catalog-only for `web-atlas` | Observations may exist elsewhere; operator Website analytics still unavailable shell | Production workspace migration |
...
| FindingsIndex (`/findings`) | REAL | `FindingReadService` | `FindingsIndex` Livewire | Acknowledge/resolve on model | `DemoRealityFinalConvergenceTest` | session-chrome filters/flash | — | — |
...
| Dashboard recentValue | UNAVAILABLE | `[]` on Dashboard | `/` dashboard | n/a | Convergence test | none | No Atlas narrative | Wire real projection if product wants |
...
| DemoState session | DEMO | `DemoState` | operator chrome / catalog | Session | Demo reset tests | session-chrome | — | Never Findings source of truth |
