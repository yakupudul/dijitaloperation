...
| Production operator screens do not read DemoState/fixtures as KPI truth | **DONE** (catalog fixtures remain for Atlas string ids only) | `GscSpecialistBindingResolver` / `Ga4SpecialistBindingResolver` require `ctype_digit`; numeric ids never `DemoCatalog`. Tests: `tests/Feature/TrackA/ProductionOperatorDemoIsolationTest.php`. `DemoState` is still used for flash/period chrome — not Finding/KPI truth |
