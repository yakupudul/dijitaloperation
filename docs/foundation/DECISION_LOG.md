...
- **Karar:**
  ...
  4. Named operator routes, OAuth callbacks (`/integrations/google/callback`, `/integrations/meta/callback`), and operator login (`/login`) stay on the product origin. Filament login is `/admin/login` and is not the operator sign-in.
