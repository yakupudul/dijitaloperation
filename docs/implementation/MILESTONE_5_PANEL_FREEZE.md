...
| Agents | PARTIAL → operational execution REAL | Code registry under `/app/settings/ai/agents` (7 specialists). Prompt 50: Agent execution **REAL** for operational specialists (Website SEO, Brand Discovery, Google Ads, Meta Ads). GBP / GA4 / GSC remain **designed** (no live execution claimed). Raw DB Agent access **FORBIDDEN**. Prompt 51: Intelligence Memory **architecture/contracts REAL**. Prompt 54: Intelligence Retrieval **REAL** on Website path (`MEMORY_CONTEXT_JSON`); other operational Agents pending wire-up. Specs: `AI_AGENT_PRODUCTION_EXECUTION.md`, `INTELLIGENCE_MEMORY_ARCHITECTURE.md`, `INTELLIGENCE_RETRIEVAL_LAYER.md`. |
...
| Intelligence Memory Architecture | REAL | Prompt 51: three layers Brand/Sector/Skill; policy gateway; provenance/temporal/access contracts; **no** generic `memories` table; **no** vectors/embeddings. Prompt 52 Brand Experience content. Prompt 53 Sector Learning pipeline **REAL**. Prompt 54 retrieval/injection **REAL** (typed pack + gateway). Spec: `INTELLIGENCE_MEMORY_ARCHITECTURE.md`, `SECTOR_LEARNING_PRIVACY.md`, `INTELLIGENCE_RETRIEVAL_LAYER.md`. |
...
| Sector Memory Privacy Contract | REAL | Prompt 53: `ProductionSectorLearningPrivacyGate` + `sector_privacy_v1` policy (`SectorLearningPrivacyPolicy`); disclosure-control thresholds (5/5 cohort, 3/3 cell, 10/10 numeric, 0.20 max share); **NOT** formal k-anonymity/DP; no privacy score; consumer contributor IDs **FORBIDDEN**. Policy: `SECTOR_MEMORY_PRIVACY_POLICY.md`. |
| Sector Learning Pipeline | REAL | Prompt 53: projection → bounding → aggregation → artifact release; Brand Experience **only** source; Spec: `SECTOR_LEARNING_PRIVACY.md` |
| Sector Learning Artifacts | REAL | `sector_learning_artifacts` + `sector_learning_revisions` + restricted `sector_learning_lineage_entries`; consumer DTO via `SectorMemoryReadService`; `SectorPatternRetriever` injects consumer DTO via Prompt 54 |
| Intelligence Retrieval Layer | REAL | Prompt 54: `IntelligenceRetrievalService` + `intelligence_retrieval_v1`; typed `IntelligenceContextPack`; gateway `resolveMemoryContextPack` real; Website `MEMORY_CONTEXT_JSON`; **no** scores/vectors/fine-tuning/RetrievalV2; eval Prompt 55. Spec: `INTELLIGENCE_RETRIEVAL_LAYER.md` |
| Memory Pack / Agent Injection | REAL | Prompt 54 server-side typed pack; bounded selection; Evidence authority preserved; Agent memory tools **FORBIDDEN** |
| Knowledge / Skill Memory Contract | REAL | Customer-free guard; references Skill/Playbook; no duplicate truth |
| Vector DB / Embeddings / Similarity Search | NOT IMPLEMENTED | Explicitly **not** used by Prompt 54 retrieval (lexicographic + structured match only) |
