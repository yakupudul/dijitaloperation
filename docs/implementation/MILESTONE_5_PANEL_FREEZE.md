...
| Intelligence Memory Architecture | REAL | Prompt 51: three layers Brand/Sector/Skill; policy gateway; provenance/temporal/access contracts; **no** generic `memories` table; **no** vectors/embeddings. Prompt 52 Brand Experience content. Prompt 53 Sector Learning pipeline **REAL**; Retrieval **NOT YET** (54). Spec: `INTELLIGENCE_MEMORY_ARCHITECTURE.md`, `SECTOR_LEARNING_PRIVACY.md`. |
...
| Sector Learning Artifacts | REAL | `sector_learning_artifacts` + `sector_learning_revisions` + restricted `sector_learning_lineage_entries`; consumer DTO via `SectorMemoryReadService`; `ReleasedSectorMemoryContextProvider` (refs only; injection Prompt 54) |
