...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('meta_integration_discovery_attempts')) {
            return;
        }

        Schema::create('meta_integration_discovery_attempts', function (Blueprint $table) {
            $table->id();
            $table->foreignId('integration_id')->constrained('core_integrations')->cascadeOnDelete();
            $table->string('phase'); // businesses | ad_accounts
            $table->string('connector')->default('meta_ads');
            $table->unsignedBigInteger('business_resource_id')->nullable();
            $table->string('edge')->nullable(); // owned_ad_accounts | client_ad_accounts | me_businesses
            $table->string('status');
            ...
            $table->string('graph_api_version')->nullable();
            ...
            $table->index(['integration_id', 'phase', 'created_at']);
    ...
    {
        Schema::dropIfExists('meta_integration_discovery_attempts');
    }
};
