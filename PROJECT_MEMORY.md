...
Website Digital Assets are an **asset-capability** collection root on the shared engine: no External Resource binding. Operator Website production collection is explicit `providerSources` (`WEBSITE_DIRECT` / `DOMAIN_DNS_TLS` / `PAGESPEED_TECHNICAL`); null `providerSources` on a website-anchored Google run must **not** auto-enqueue Website/DataForSEO families. DataForSEO is **paid external enrichment**: agency Integration credentials, Website-asset-scoped facts, never a Digital Asset or routine scheduler. A charged POST is preceded by a durable `paid_attempt_started` checkpoint; an unresolved attempt fail-closes that DatasetRun (`CHARGE_UNKNOWN`) even if request-shaping inputs recompute a different fingerprint. Only a new DatasetRun may issue a different charged request. Public Discovery stays bounded to public Website/context + consented DataForSEO competitor **candidates**. Legacy Website/SEO Evidence collectors remain; the engine path writes Data Pool facts only. Both Website crawl collection and DataForSEO engine-driven enrichment are **PARTIAL** (coded/tested, live UAT required) on the stacked child of PR #203.
