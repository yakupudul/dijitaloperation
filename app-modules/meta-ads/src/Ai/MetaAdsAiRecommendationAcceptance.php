 ...
 */
final class MetaAdsAiRecommendationAcceptance
{
    /**
    ...
    public function acceptDraft(DigitalAsset $asset, int $findingId, ?Evidence $insight = null): array
    {
        if ($asset->type !== 'meta_ads') {
            throw new InvalidArgumentException('AI recommendation acceptance requires a meta_ads Digital Asset.');
        }

        $insight ??= app(MetaAdsAiGuidanceService::class)->latestSuccessfulInsight($asset);
        ...
        if (! $finding instanceof Finding) {
            throw new InvalidArgumentException('Finding not found for this Meta Ads asset.');
        }
